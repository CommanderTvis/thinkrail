...
export const AgentStatusReportSchema = Type.Object({
	...
	/** Where Claude Code itself says the session's transcript is, honouring its own CLAUDE_CONFIG_DIR. */
...
export const AgentTokenUsageSchema = Type.Object({
	input: Type.Number(),
	output: Type.Number(),
	cacheRead: Type.Number(),
	cacheWrite: Type.Number(),
});
export type AgentTokenUsage = Static<typeof AgentTokenUsageSchema>;
...
export const ClaudeCodeStatusPushSchema = Type.Object({
	...
	/** Read by the host from the session's own transcript, never sent by the hook. */
	usage: Type.Optional(AgentTokenUsageSchema),
});
