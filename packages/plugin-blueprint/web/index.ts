...
export default definePluginWeb<typeof blueprintContract>({
	activate(ctx) {
		...
		function openBlueprintChat(workspaceId: string): void {
			void openBlueprintPair(workspaceId).catch((error: unknown) => {
				ctx.notify(
					"error",
					"Could not open Blueprint chat",
					error instanceof Error ? error.message : String(error),
				);
			});
		}

		function BlueprintFileRedirect({ workspaceId }: { workspaceId: string }) {
			useEffect(() => {
				for (const editor of ctx.editors.list(workspaceId)) {
					if (editor.kind === "file" && editor.path === BLUEPRINT_FILE)
						ctx.editors.close(editor.id);
				}
				openBlueprintChat(workspaceId);
			}, [workspaceId]);
			return null;
		}
		...
		ctx.fileViewer({
			...
			component: BlueprintFileRedirect,
			open: (workspaceId) => {
				openBlueprintChat(workspaceId);
				return true;
			},
