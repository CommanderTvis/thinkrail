...
async function restoreCodex(page: Page) {
	await requestOverWire(page, "settings.update", {
		config: { plugins: { codex: { enabled: false, command: "codex", permissionMode: "default" } } },
	});
}
...
test("Codex settings persist default permissions and menu permissions override them", async ({
	...
	try {
		...
		await page.getByTestId("codex-permission-mode-yolo").click();
		await expect(picker).toHaveText("Bypass approvals and sandbox (--yolo)");
		await page.reload();
		await page.getByTestId("open-settings").click();
		await page.getByTestId("settings-nav-codex").click();
		await expect(picker).toHaveText("Bypass approvals and sandbox (--yolo)");
		...
		await picker.click();
		await expect(page.getByRole("menuitemradio", { checked: true })).toHaveText(
			"Bypass approvals and sandbox (--yolo)",
		);
		...
		await expect
			.poll(args)
			.toEqual(["-c", expect.stringContaining("mcp_servers.thinkrail.url="), "--yolo"]);
		await launcher.click({ button: "right" });
