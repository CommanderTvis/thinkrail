...
function TabStrip({
	...
	interface TabGroupItem {
		pane: LayoutTabPane;
		tabs: Array<{ tab: LayoutTab; index: number }>;
	}
	type StripItem =
		| { kind: "tab"; tab: LayoutTab; index: number }
		| { kind: "group"; group: TabGroupItem };

	const stripItems: StripItem[] = [];
	if (nested && ownGroup?.panes && ownGroup.panes.length > 0) {
		let currentGroup: TabGroupItem | null = null;
		tabs.forEach((tab, index) => {
			const pane = paneForTab(ownGroup, tab.id);
			if (pane) {
				if (currentGroup && currentGroup.pane.id === pane.id) {
					currentGroup.tabs.push({ tab, index });
				} else {
					const nextGroup: TabGroupItem = { pane, tabs: [{ tab, index }] };
					currentGroup = nextGroup;
					stripItems.push({ kind: "group", group: nextGroup });
				}
			} else {
				currentGroup = null;
				stripItems.push({ kind: "tab", tab, index });
			}
		});
	} else {
		tabs.forEach((tab, index) => {
	...
	const renderTab = (tab: LayoutTab, index: number, inGroup = false) => (
		<WorkbenchTab
			key={tab.id}
			tab={tab}
			index={index}
		...
		>
			...
			>
				<div
					...
					className={
						vertical
							? `flex min-h-0 flex-1 flex-col items-stretch overflow-x-hidden overflow-y-auto [scrollbar-width:thin] ${nested ? "gap-2" : ""}`
				...
				>
					{stripItems.map((item) => {
						if (item.kind === "tab") {
							return renderTab(item.tab, item.index, false);
						}
						if (item.group.tabs.length < 2) {
							return item.group.tabs.map(({ tab, index }) => renderTab(tab, index, false));
						}
						const isGroupActive = item.group.tabs.some(({ tab }) => tab.id === selectedId);
						return (
							<div
								key={item.group.pane.id}
								data-testid="tab-pane-group"
								data-active={isGroupActive || undefined}
								className="group/pane relative flex w-full flex-col divide-y divide-border-default rounded-[var(--radius-md)] border border-transparent border-l-2 border-l-primary transition-colors hover:bg-control-bg-hovered data-[active=true]:border-border-default data-[active=true]:border-l-primary data-[active=true]:bg-control-bg-selected"
							>
								{item.group.tabs.map(({ tab, index }) => renderTab(tab, index, true))}
							</div>
						);
					})}
					{acceptsAppend ? (
				...
				<ContextMenuTrigger asChild>
					<div
						...
						className={
							nested
								? inGroup
									? "group relative flex w-full shrink-0 items-center text-text-muted transition-colors group-hover/pane:text-text-default group-data-[active=true]/pane:text-text-default has-[[role=tab]:focus-visible]:ring-2 has-[[role=tab]:focus-visible]:ring-inset has-[[role=tab]:focus-visible]:ring-primary data-[dragging]:opacity-40"
									: "group relative flex w-full shrink-0 items-center rounded-[var(--radius-md)] border border-transparent text-text-muted transition-colors hover:bg-control-bg-hovered hover:text-text-default has-[[role=tab]:focus-visible]:ring-2 has-[[role=tab]:focus-visible]:ring-inset has-[[role=tab]:focus-visible]:ring-primary data-[active=true]:border-border-default data-[active=true]:bg-control-bg-selected data-[active=true]:text-text-default data-[dragging]:opacity-40"
								: vertical
