...
import { type FrontmatterProperty, parseFrontmatter, withFrontmatter } from "@/panels/frontmatter";
	...
	return (
		<div className="flex min-w-0 flex-1 flex-col py-2">
			{pairs.map(([key, value], index) => (
				<div key={key} className="flex min-w-0 items-start gap-8">
...
/** Suggestions, not constraints: the doc may use any vocabulary, but the app may know a closed set. */
const STATUS_SUGGESTIONS = ["draft", "active", "stale", "done", "deprecated"];
...
function ListValue({ items, onCommit }: { items: string[]; onCommit: (next: string[]) => void }) {
	...
	// A key by occurrence rather than position: stable for a reorder-free list, and still unique when
	// two tags share a value, which plain position never was.
...
export function FrontmatterProperties({
	...
	const valueSuggestions: Record<string, readonly string[]> = {
		status: STATUS_SUGGESTIONS,
		...(typeSuggestions ? { type: typeSuggestions } : {}),
	};
							...
							>
								...
								{Array.isArray(property.value) ? (
									...
									<TextValue
										...
										suggestions={valueSuggestions[property.key]}
									/>
								)}
								<button
									type="button"
