...
/** Whether a tool's body is on screen: in a visible region, an unfolded group, and that group's shown tab. */
export function isToolShowing(
	document: WorkspaceLayoutDocument,
	attention: LayoutAttention,
	tool: LayoutToolId,
): boolean {
	for (const region of ["left", "right", "bottom"] as const) {
		if (!document[region].visible) continue;
		for (const group of document[region].groups) {
			if (group.folded) continue;
			const selectedId = readLayoutSelection(attention, group.id);
			const shown = group.tabs.find((tab) => tab.id === selectedId) ?? group.tabs[0];
			if (shown?.kind === "tool" && shown.tool === tool) return true;
		}
	}
	return false;
}
