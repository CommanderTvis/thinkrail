...
import { pluginAssetBase, usePluginRegistry } from "../registry";
...
function buildConfigProjection(state: ReturnType<typeof useAppStore.getState>): AppConfig {
	return {
		...
		analyticsConsentConfirmed: state.analyticsConsentConfirmed,
...
export function createWebContext<C extends PluginContract>(
	...
	return {
		...
		editors: {
			...
			async open(workspaceId, path, options) {
				...
				if (options?.keyPath !== undefined) {
					useAppStore.getState().requestFileFocus(workspaceId, path, options.keyPath);
		...
		async openTerminal(workspaceId, options) {
			...
			const known = store.terminalsByWorkspace[workspaceId]?.find((tab) => tab.tabKey === tabKey);
			const targetGroupId =
				options?.groupId ?? store.layoutAttentionByWorkspace[workspaceId]?.lastFocusedCenterGroupId;
			if (known) {
				store.enqueueLayoutIntent({
					...
					tabKey,
					title: known.title,
					...(targetGroupId ? { targetGroupId, targetArea: "center" as const } : {}),
				});
				store.setActiveTerminalTab(workspaceId, tabKey);
			} else {
				store.addTerminal(workspaceId, options?.command, targetGroupId, "center", true, tabKey);
			}
			return { tabKey };
		},
		async openChat(workspaceId, options) {
			if (options?.sessionId) {
				const store = useAppStore.getState();
				const isDeleted = store.deletedSessionsByWorkspace[workspaceId]?.[options.sessionId];
				if (!isDeleted) {
					try {
						await openChatInTab(workspaceId, options.sessionId);
						const after = useAppStore.getState();
						const tabOpen = (after.tabsByWorkspace[workspaceId] ?? []).some(
							(t) => t.kind === "chat" && t.sessionId === options.sessionId,
						);
						if (tabOpen) return { sessionId: options.sessionId };
					} catch {
						// session cannot be recovered - fall through to create new one
					}
				}
			}
		...
		assetUrl(path) {
			const base = pluginAssetBase(entry, getTransport().httpBase());
			if (base === undefined) throw new Error(`plugin ${id} declares no assets`);
			return `${base}/${path}`;
		},
