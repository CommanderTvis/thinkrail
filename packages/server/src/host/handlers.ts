...
const handlers: Record<string, Handler> = {
	...
	"claudeConfig.account": (params) => {
		requireClaudeCode();
		const p = params as { refresh?: boolean };
		return readClaudeAccount(getConfig().claudeCommand, p.refresh === true);
	},
	"claudeConfig.mcpList": async (params) => {
		requireClaudeCode();
		const p = params as { workspaceId: string };
		const root = getWorkspace(p.workspaceId).worktreePath;
		const declared = new Set(
			resolveClaudeConfig(p.workspaceId, root)
	...
	"visualization.get": (params) => {
		const { workspaceId, tabKey } = params as { workspaceId: string; tabKey: string };
		return getVisualization(workspaceId, tabKey);
	},
