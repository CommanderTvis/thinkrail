...
export interface TerminalAccessoryApi {
	...
	/** Reads the last `lines` lines of the terminal's buffer. */
...
export interface AgentLauncher {
	...
	/** Models the launcher offers a choice of, when it supports more than one. */
...
export interface PluginWebContext<C extends PluginContract> {
	...
	editors: {
		...
		open(
			workspaceId: string,
			path: string,
			options?: { line?: number; preview?: boolean; raw?: boolean },
	 ...
	 */
	openChat(workspaceId: string, options?: { prompt?: string }): Promise<{ sessionId: string }>;
