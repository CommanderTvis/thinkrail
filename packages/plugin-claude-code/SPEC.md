...
## Core's "external file" editing reaches this plugin by name, not by capability

`readFile`/`writeFile` read/write a file outside any worktree — a `~/.claude/settings.json`, say — for the
same generic `external-file` tab kind core already has (worktree files use `fs.readFile`/`fs.writeFile`
instead). There is no `PluginWebContext` capability for "a plugin that can serve an external-file read" —
today, only this plugin ever produces such a path (`ClaudeConfigParts.tsx`'s `useOpenSource`) — so the four
core call sites that read/write an `external-file` tab (`apps/web/src/shell/WorkspaceWorkbench.tsx`,
`panels/FilePane.tsx`, `panels/openTabs.ts`, `panels/fileSave.ts`) call `pluginMethodName(CLAUDE_CODE_ID,
"readFile" | "writeFile")` directly, importing this plugin's `manifest.ts` for the id constant. This is a
real, named coupling from core to one specific plugin — not a capability a second plugin could also
implement — accepted here rather than inventing a generic "external file source" capability under time
pressure; `panels/SPEC.md`'s "Editing a file" section names the same gap from the other side. The same
hook's click opens the file without jumping to a `keyPath`'s line, as the core-owned pane once did:
`editors.open` carries a line number, not a JSON key path, and nothing resolves one to the other yet
(`panels/SPEC.md`, "A key path names a value") — a resolver belongs here if losing that jump is felt.
...
Core's own roster/tab-icon lookup (`plugins/registry/icons.ts`'s `pluginIcon("claude")`) resolves the
same mark independently, through the file-icons plugin's `fileIcon` slot rather than by importing
anything from this plugin — the two draw the identical glyph without either depending on the other.
...
relativize-against-cwd logic is unchanged and still covered (`web/terminalCwd.test.ts`); what's gone is
being able to browse the worktree without leaving the terminal. `apps/web/src/panels/terminalCwd.ts` keeps
a separate, smaller copy of `attachPath` alone (no `cwdLabel`, which is Claude-only) — core's own drag-a-
...
- **Owns:** `manifest.ts`, `contracts.ts` (the Claude Code, IDE-bridge, and agent-status domain types,
  ...
  route and store, the transcript interrupt watch, activation), `web/` (the configuration pane, tab decorations, launcher, terminal
  accessory — `ClaudeTerminalFacts` (fact chips, plan, attach, picker drive) plus `ClaudeCodeChip` (the
  install offer) — editor-event bridge, `terminalCwd.ts`'s own `attachPath`+`cwdLabel`), `build-support.ts`,
...
settings-sourced command line reaching `pluginUninstallPlan` instead of core config. `web/terminalCwd.
test.ts` pins `attachPath`/`cwdLabel` (moved from the old `apps/web/src/panels/terminalCwd.test.ts`).
