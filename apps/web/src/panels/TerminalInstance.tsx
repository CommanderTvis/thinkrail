		...
		<>
			...
			{open ? (
				<div
					...
					className="absolute inset-x-0 bottom-full z-20 mb-4 max-h-[40vh] overflow-auto rounded-[var(--radius-sm)] border border-border-default bg-container-elevated-bg p-8"
				>
...
export default function TerminalInstance({ tabKey, workspaceId, initialCommand }: Props) {
	...
	useEffect(() => {
		...
		const onData = term.onData((data) => {
			const id = serverIdRef.current;
			// Every keystroke, paste and mouse report enters the pty here, and a picker being driven owns
			// the pty until it is done — see panels/SPEC.md.
			if (id && !driving.current)
			...
			void drive(
				...
				.then((outcome) => {
					if (outcome === "switched") {
						...
						const current = selectClaudeCodeStatus(state, workspaceId, tabKey);
						if (what === "effort" && current)
							state.setClaudeCodeStatus(workspaceId, tabKey, current.status, { effort: choice });
						return;
					}
					pushToast({
						variant: "error",
				...
				.catch(() =>
					pushToast({
						variant: "error",
						title: `Couldn't switch the ${what}`,
						message: "The terminal stopped answering while the picker was open.",
					}),
				)
				// The pane is sealed until this runs, so it runs however the drive ended.
				.finally(() => {
					driving.current = false;
					setDrivingWhat(null);
				});
		},
		[pushToast, tabKey, workspaceId],
	);
	...
	return (
		...
		>
			<QuietScrollFrame
				...
				surface="terminal"
				edges={scrollEdges}
				className={`mx-12 mt-12 min-h-0 flex-1 ${claudeHere ? "mb-8" : "mb-12"}`}
			>
			...
			{claudeHere ? (
				<div
					...
					className="relative mx-12 mb-8 flex shrink-0 flex-wrap items-center gap-4"
				>
					{facts.map((fact) =>
						fact.kind === "model" ? (
			...
			{drivingWhat ? (
				<div
					...
					className="absolute inset-0 z-30 flex cursor-progress flex-col items-center justify-center bg-overlay"
				>
