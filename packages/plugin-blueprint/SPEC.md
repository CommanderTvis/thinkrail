...
- **Reopening a closed terminal author.** `openTerminal`'s `tabKey` places and selects a terminal, and
  ...
  exactly what the Claude launcher's own `terminalCommand({ resume })` needs to compose a resume/continue
  line. `blueprintOpen.ts`'s `restoreAuthor` calls it with the author's `tabKey` and `agentSessionId`
  when the author's tab is no longer in `ctx.host().terminals[workspaceId]`, the same launcher choice
  `BlueprintStartDialog` used to start it. A chat author has no equivalent — W13 starts a session, it
  does not resume one by id — so a chat author whose tab is gone still falls back to the raw file.
