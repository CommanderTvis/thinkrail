...
function blockFrom(view: MarkdownView, start: number): SurfaceBlock | null {
...
export function readSurfaceBlock(specText: string): SurfaceBlock | null {
	...
	return start === -1 ? null : blockFrom(view, start);
}
...
export function readSurfaceBlocks(specText: string): SurfaceBlock[] {
	const view = markdownView(specText);
	const blocks: SurfaceBlock[] = [];
	let cursor = 0;
	while (cursor < view.lines.length) {
		const labelled = indexFrom(view.lines, declaredByLabel, cursor);
		const start = labelled === -1 ? indexFrom(view.lines, declaredInProse, cursor) : labelled;
		if (start === -1) break;
		const block = blockFrom(view, start);
		if (block === null) break;
		blocks.push(block);
		cursor = start + block.text.split("\n").length;
	}
	return blocks;
}
...
const SURFACE_SUBPATH = /\(`([^`]+)`\)/;

function surfaceLabelText(block: SurfaceBlock): string {
	const first = block.text.split("\n")[0] ?? "";
	if (block.heading) return first.replace(/^#{1,6}\s+/, "");
	return BULLET_LABEL.exec(first)?.[1] ?? first;
}

export function surfaceSubpath(block: SurfaceBlock): string {
	return SURFACE_SUBPATH.exec(surfaceLabelText(block))?.[1] ?? ".";
}

function exportSubpath(surfacePath: string): string {
	const withoutIndex = surfacePath.replace(/(^|\/)index\.ts$/, "");
	const withoutSrc = withoutIndex.replace(/^src\/?/, "");
	return withoutSrc === "" ? "." : `./${withoutSrc}`;
}
...
export async function checkSpecSurfaces(inputRoot: string): Promise<SurfaceCheckReport> {
	...
	for (const entry of entries) {
		...
		const body = parseFile(entry.content).body;
		const enrolled = list(entry.frontmatter, FIELDS.tags).includes(PUBLIC_SURFACE_TAG);
		const surfacePaths = list(entry.frontmatter, SURFACES_FIELD);

		if (surfacePaths.length > 0) {
			if (!enrolled) {
				report.skipped.push({ path: entry.path, reason: "not enrolled" });
				continue;
			}
			report.enrolled++;
			const blocks = readSurfaceBlocks(body);
			if (blocks.length !== surfacePaths.length) {
				report.violations.push(
					`${entry.path}: tagged ${PUBLIC_SURFACE_TAG} with ${surfacePaths.length} ` +
						`\`surfaces:\` entries but found ${blocks.length} public-surface block(s)`,
				);
				continue;
			}
			const expectedSubpaths = surfacePaths.map(exportSubpath);
			const declaredSubpaths = blocks.map(surfaceSubpath);
			for (const subpath of declaredSubpaths) {
				if (!expectedSubpaths.includes(subpath)) {
					report.violations.push(
						`${entry.path}: public-surface block (\`${subpath}\`) does not match any \`surfaces:\` entry`,
					);
				}
			}
			for (const [surfaceIndex, surfacePath] of surfacePaths.entries()) {
				const block = blocks[declaredSubpaths.indexOf(expectedSubpaths[surfaceIndex] ?? "")];
				if (block === undefined) {
					report.violations.push(
						`${entry.path}: surface ${surfacePath} has no matching public-surface block`,
					);
					continue;
				}
				if (!isBareNameList(block)) {
					report.violations.push(
						`${entry.path}: surface ${surfacePath}'s public surface is not a bare identifier list`,
					);
					continue;
				}
				const barrel = resolve(dirname(specFile), surfacePath);
				if (!fileExists(barrel)) {
					report.violations.push(`${entry.path}: surface ${surfacePath} does not exist`);
					continue;
				}
				candidates.push({
					specPath: `${entry.path} (${surfacePath})`,
					barrel,
		...
		const block = readSurfaceBlock(body);
