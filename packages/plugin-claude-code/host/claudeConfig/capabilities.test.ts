...
import { pluginStatus, pluginStatusMaintained, shippedVersion } from "./plugin";
...
describe("plugin registration", () => {
	test("an entry the user already approved is brought back into line without asking again", async () => {
		writeFileSync(
		...
		writeFileSync(
			...
			JSON.stringify({
				version: 1,
				plugins: { "thinkrail@thinkrail": [{ scope: "user", version: shippedVersion() }] },
			}),
			"utf8",
		);
		expect(pluginStatus().state).toBe("outdated");
		expect((await pluginStatusMaintained("/nonexistent/claude")).state).toBe("enabled");
		// And the second reading is a no-op rather than a rewrite loop.
		expect(pluginStatus().state).toBe("enabled");
	});

	test("nothing registered still asks", async () => {
		writeFileSync(join(home, "settings.json"), "{}", "utf8");
		expect((await pluginStatusMaintained("/nonexistent/claude")).state).toBe("absent");
	});
});

describe("project instructions", () => {
	const labels = (root: string) =>
		resolveClaudeConfig("ws", root)
			.context.filter((layer) => layer.origin.scope === "project")
			.map((layer) => layer.label);

	test("AGENTS.md stands in for a missing CLAUDE.md", () => {
		expect(labels(project({ "AGENTS.md": "agents" }))).toEqual(["AGENTS.md"]);
	});

	test("CLAUDE.md wins over AGENTS.md", () => {
		expect(labels(project({ "AGENTS.md": "agents", "CLAUDE.md": "claude" }))).toEqual([
			"CLAUDE.md",
		]);
	});
});
