...
import type { ClaudeConfigScope } from "../../contracts";
...
/** Claude Code reads the root `AGENTS.md` when there is no root `CLAUDE.md` — see SPEC.md. */
function projectInstructionsPath(root: string): string {
	const claude = join(root, "CLAUDE.md");
	const agents = join(root, "AGENTS.md");
	return !existsSync(claude) && existsSync(agents) ? agents : claude;
}
...
export function instructionPaths(root: string): ScopedPath[] {
	return [
		...
		{ scope: "project", path: projectInstructionsPath(root) },
