...
tags: [v1, plugins, public-surface-checked]
surfaces: [src/index.ts, src/host/index.ts, src/web/index.ts]
---
...
- **Entries:** three, with no cross re-export. The root (`@thinkrail/plugin-api`) carries
  what both rings and the plugin share — the identity helpers, `PluginManifest`/`definePluginManifest`, the
  `PluginContract`/`definePluginContract` vocabulary and its `Static` projections, the pi-free
  `PluginToolDefinition`/`definePluginTool`, and re-exports of the roster and agent-record types owned by
  `@thinkrail/contracts`. `./host` carries `PluginHostContext`/`definePluginHost` and the types its members
  use. `./web` carries `PluginWebContext`/`definePluginWeb` and the UI contribution types.
- **Public surface (`.`):** `PLUGIN_API_GENERATION`, `PLUGIN_ID_PATTERN`, `isPluginId`,
  `pluginMethodName`, `pluginChannelName`, `pluginRoute`, `pluginToolId`, `parsePluginToolId`,
  `pluginPreferenceKey`, `pluginStateFile`, `PluginDependency`, `PluginPiBlock`, `PluginManifest`,
  `definePluginManifest`, `PluginMethodSpec`, `PluginChannelSpec`, `PluginContract`,
  `definePluginContract`, `MethodParams`, `MethodResult`, `ChannelPayload`, `PluginSettings`,
  `TerminalRef`, `PluginToolSurface`, `PluginToolContext`, `PluginToolResult`, `PluginToolDefinition`,
  `definePluginTool`, `PluginRosterEntry`, `PluginContributions`, `PluginSideToolContribution`,
  `PluginFileViewerContribution`, `PluginStatus`, `PluginOrigin`, `TerminalAgentRecord`.
- **Public surface (`./host`):** `PluginLogger`, `PluginDisposer`, `PluginCall`, `TerminalEvent`,
  `WorkspaceEvent`, `RevivePrefill`, `GitRunOptions`, `GitRunResult`, `DependencyHandle`,
  `PluginHostContext`, `PluginHostActivate`, `PluginHostModule`, `definePluginHost`.
- **Public surface (`./web`):** `PluginWebLogger`, `PluginDisposer`, `HostProjection`, `EditorRef`,
  `EditorEvent`, `CompanionHost`, `TerminalAccessoryApi`, `AgentLauncher`, `FileViewerProps`,
  `FileViewerRegistration`, `SettingsSectionRegistration`, `SideToolRegistration`,
  `CompanionRegistration`, `TabDecoration`, `TabRef`, `WorkspaceActionRegistration`,
  `WorkspaceScopedActionRegistration`, `ProjectScopedActionRegistration`,
  `TerminalAccessoryRegistration`, `CoreSlots`, `ToolStatus`, `ToolRenderProps`, `ToolRenderer`,
  `ToolRendererOptions`, `PluginWebContext`, `PluginWebActivate`, `PluginWebModule`, `definePluginWeb`.
...
- **Enrolment:** `public-surface-checked`, pinned to the three entries above via this spec's `surfaces:`
  frontmatter list.

### Reference documentation

Every export of the three entries carries a TSDoc block: what the symbol is for a plugin author, the
rules that bind it, and `@param`/`@returns`/`@example`/`@remarks` where they earn their place. This is the
one deliberate exception to the near-zero-comments rule stated under Spec graph in `AGENTS.md`. It exists
because this package's readers are external plugin authors, who read the types their editor shows them
rather than this spec — the spec stays the design record and the source of every rule, and the TSDoc is
its per-symbol reference, derived from it rather than duplicating its history or rationale.

TypeScript 7 ships no JavaScript checker, so the reference is generated from an isolated install pinned to
TypeScript 5: `bun add -d typedoc@0.28.14 typescript@5.9.3` in a scratch directory, then
`typedoc --tsconfig packages/plugin-api/tsconfig.api.json --entryPoints src/index.ts src/host/index.ts
src/web/index.ts`. The committed `.api` reports (see Stability, above) already carry the doc comments
verbatim, since the printer resolves each export's full declaration text.

### Names

| entry | exported identifiers |
| --- | --- |
| `.` | `PLUGIN_API_GENERATION`, `PLUGIN_ID_PATTERN`, `isPluginId`, `pluginMethodName`, `pluginChannelName`, `pluginRoute`, `pluginToolId`, `parsePluginToolId`, `pluginPreferenceKey`, `pluginStateFile`, `PluginDependency`, `PluginPiBlock`, `PluginManifest`, `definePluginManifest`, `PluginMethodSpec`, `PluginChannelSpec`, `PluginContract`, `definePluginContract`, `MethodParams`, `MethodResult`, `ChannelPayload`, `PluginSettings`, `TerminalRef`, `PluginToolSurface`, `PluginToolContext`, `PluginToolResult`, `PluginToolDefinition`, `definePluginTool`, `PluginRosterEntry`, `PluginContributions`, `PluginSideToolContribution`, `PluginFileViewerContribution`, `PluginStatus`, `PluginOrigin`, `TerminalAgentRecord` (the last seven re-exported from `@thinkrail/contracts`) |
| `./host` | `PluginLogger`, `PluginDisposer`, `PluginCall`, `TerminalEvent`, `WorkspaceEvent`, `RevivePrefill`, `GitRunOptions`, `GitRunResult`, `DependencyHandle`, `PluginHostContext`, `PluginHostActivate`, `PluginHostModule`, `definePluginHost` |
| `./web` | `PluginWebLogger`, `PluginDisposer`, `HostProjection`, `EditorRef`, `EditorEvent`, `CompanionHost`, `TerminalAccessoryApi`, `AgentLauncher`, `FileViewerProps`, `FileViewerRegistration`, `SettingsSectionRegistration`, `SideToolRegistration`, `CompanionRegistration`, `TabDecoration`, `TabRef`, `WorkspaceActionRegistration`, `WorkspaceScopedActionRegistration`, `ProjectScopedActionRegistration`, `TerminalAccessoryRegistration`, `CoreSlots`, `ToolStatus`, `ToolRenderProps`, `ToolRenderer`, `ToolRendererOptions`, `PluginWebContext`, `PluginWebActivate`, `PluginWebModule`, `definePluginWeb` |

`PluginLogger` and `PluginWebLogger` are independently declared with the same shape rather than one
re-exported from the other, since the public-surface rule above forbids a cross re-export between `/host`
and `/web`. `PluginDisposer` is likewise declared once per entry that needs it.
...
The `PluginManifest` type and the `definePluginManifest` identity helper, both from
`@thinkrail/plugin-api`, are the same fields with two carriers: a builtin plugin exports a
`definePluginManifest`-wrapped value from a typebox-free module, and an external plugin ships the same
fields as `thinkrail-plugin.json`.
...
| `description` | optional one sentence on what the plugin does, shown under the label in Settings |
...
| `assets` | optional directory the host serves or hands to the plugin — for either origin, static files under it are served at `/plugin/<id>/<assets>/…` (`plugins/index.ts`'s `serveRoute`) and reachable from the web half via `PluginWebContext.assetUrl` (W19) |
| `contributes` | the statically known parts: side tools with label, icon, default side and whether they need git (`requiresGit`); file viewers with extensions and read strategy |
...
label, description, icon, version, origin, toggle, declared contributions, whether the plugin modifies the system
...
A plugin declares one `PluginContract` value, wrapped in `definePluginContract`, that the host half
imports and validates against: `PluginMethodSpec` entries with typebox params and result schemas,
`PluginChannelSpec` entries, a settings schema, and a wire version. The web half never imports that value
— only its shape, via `import type` and the `MethodParams`, `MethodResult`, `ChannelPayload`, and
`PluginSettings` projections, so `definePluginWeb<C>({ activate })` takes `C` purely as a type parameter.
A builtin plugin's manifest is a value import (typebox-free, from `./manifest`), which is what lets the
web loader's eager registry hold every builtin's manifest before any plugin activates. What the web half
needs at runtime instead of the contract value — a channel's kind, its snapshot method, its key fields,
and the wire version to check against — travels on `PluginRosterEntry` (`channels`, `wireVersion`),
built by the host from the contract it already has loaded; `createWebContext` reads those off the roster
entry, and the wireVersion check in the loader compares the bundled manifest's `wireVersion` (builtin:
the statically imported manifest; external: trusted from the roster, since the served web module and the
manifest that produced the roster row are read from the same plugin directory) against the roster's.
...
and the channel union; a roster entry carrying id, wire version, label, optional description, icon, version, origin, status
...
| W3 | select from a read projection: projects, workspaces, active workspace, context project, active editor, config, per-workspace fs revision, host platform — synchronously (`host()`), reactively (`useHost`), or observed outside React (`watchHost`, firing only when the selected value changes) | store selectors |
...
| W10 | contribute a workspace-level or project-level action (`scope: "workspace"` default, or `"project"`) | the workspace's start-actions row — the centre strip's trailing row, or under the workspace in Projects when the client keeps its tabs there; a project-scoped one also renders on the Welcome screen and after project creation |
| W11 | contribute a terminal accessory row that can write, read the buffer tail, set key encoding | `TerminalInstance` owns xterm |
| W12 | observe editor events; open (optionally bypassing every file viewer, `raw`), close, list, check, save editors; report a selection into the editor-events stream | the active-editor selector and the open helper; no selection events |
| W13 | open a terminal (placing and selecting the tab; a `tabKey` already in the host catalog is attached to instead of re-created) or chat (submitting an opening `prompt` immediately, not staging it as a draft), enter a project's default workspace, ask the host for a file picker | store actions plus `panels/defaultWorkspace.ts`, reachable only inside `panels` |
...
| W18 | scope the Changes panel to a commit, the uncommitted tree, or a branch | `diffScopeByWorkspace` is store-private state |
| W19 | build a URL for a file under the manifest's declared `assets` directory | a plugin with no assets throws rather than guessing a path |

W17 is occupied: the builtin `packages/plugin-file-icons` registers into it, and core's own
`FileTypeIcon` falls back to a plain Remix glyph only when no plugin answers.
...
`editors.open` accepts a JSON `keyPath` to reveal a declaration in the editor's current contents.
It takes precedence over `line`; an unresolved key opens at the top. The editor owns resolution so
unsaved changes cannot make a plugin's cached line number stale.

`editors.open`'s `raw` option bypasses the file-open dispatcher entirely — no viewer's `open` handling and
no binary read strategy — for a plugin whose own file viewer needs an escape hatch back to the file's
ordinary text (Blueprint's "Open raw source" surface on a rehydrated, content-less tab is the first
caller). `editors.reportSelection(editor, selection)` feeds a selection into the same core surface Monaco
and the markdown preview already report through, so a plugin's own rendered document — a passage in
Blueprint's pane, say — reaches an IDE bridge the same way selecting code does, without importing it
directly.
...
`PluginHostContext.externalFiles(provider)` registers a synchronous, workspace-scoped list of absolute
paths the plugin exposes for text editing. The host checks current providers on every absolute file read
or save; only active registrations count. This is an exact file allowlist, not directory access. Plugins
own discovery; core owns reading, compare-and-swap writes, editor buffers, and restoration.
