...
import { CLAUDE_SCOPE_WORDING, claudeEditScopes } from "../webValues";
import { errorText } from "./errorText";
import { abbreviateHomePath } from "./pathDisplay";
 ...
 * whole step exists to catch — see SPEC.md.
 */
export function createClaudeEditDialog(ctx: PluginWebContext<typeof claudeCodeContract>) {
	return function ClaudeEditDialog({
		workspaceId,
		edit,
		title,
		onClose,
		...
		// caller remounts this per edit (see its `key`), so every edit starts from here rather than
		// inheriting the previous one's scope and diff.
		...
		const choose = useCallback(
			(next: ClaudeWritableScope) => {
				...
				setError(null);
				void ctx
					.request("planEdit", { workspaceId, scope: next, edit })
					.then(setPlan)
		...
		const apply = () => {
			...
			setBusy(true);
			void ctx
				.request("applyEdit", { workspaceId, scope, edit, baseHash: plan.baseHash })
				.then(() => {
					onApplied();
					onClose();
				})
