---
id: submodule-pi-agent-engine
type: submodule-design
status: active
title: engine — pi sessions inside the ACP agent process
parent: module-pi-agent
depends-on: [module-contracts]
references: [module-spec-graph, submodule-pi-agent-acp]
tags: [v1, pi, acp]
---

## Responsibility

The `pi` engine, unchanged by the ACP migration except for where it runs: a current shared model/auth
runtime generation for pre-session work and future chats, the lifecycle of `AgentSession`s (one per ACP
session, rooted in a workspace's worktree and retaining the runtime generation they were created with),
Pi resource/skill loading (including portable cross-agent skill discovery + a pre-session skill catalog),
the **extension-UI bridge** that turns pi's `uiContext` dialog calls into an injected bridge, the
**`ask_user_question`** tool + its answer-injection path, and the **restart repair** that keeps re-opened
transcripts provider-valid.

It knows nothing about ACP. It publishes `EngineEvent`s to an injected sink and answers plain calls;
the sibling `acp` module is the only thing that turns either into protocol.

## Where the wire went

This module moved here from `packages/server/src/agent` **by `git mv`, not rewrite** — the code is the
working pi integration, and the prose below is its spec carried across with it. Two classes of claim in
it now belong elsewhere and are kept only for the pi behaviour they describe:

- **Anything naming the wire** — `PiEvent`, `SessionEventPayload`, `toWireModel`/`WireModel`,
  `model.list` / `model.refresh` / `session.create` / `session.setModel` frames, WS request failure.
  ThinkRail's wire is `packages/contracts` and the host owns it; here the same values leave as
  `EngineEvent`s and engine types. The *fail-closed model projection* the `WireModel` bullet argues for
  now lives on the client side of the protocol, in `packages/acp`.
- **Anything naming the host** — `provider.status`, in-app provider login, Central artifact watching.
  Those surfaces belong to `packages/server`; the runtime-generation machinery they drive stays here.

Reconciling that prose line by line is deliberately **not** part of the move (see `module-pi-agent`
*Later*), because rewriting a 400-line spec in the same change that moves the code it describes hides
the move in the diff.
...
- **Public surface (barrel):** the manager operations (incl. `answerQuestion` +
  ...
  Three seams exist for the sibling `acp` module and nothing else: **`setQuestionnaireAsk`** (the
  `ask_user_question` tool's answer source — it receives the session id from
  `ctx.sessionManager.getSessionId()`, so a questionnaire is answered against the session that asked
  rather than a guessed one), **`getSessionConfig(sessionId)`** (the live session's model + thinking
  level, so config options are read from the session instead of re-derived from defaults), and
  **`setSessionToolsProvider`** (below).
- **`setSessionToolsProvider` is the only tool seam, and it is deliberately whole-set.** The provider is
  called once per `createAgentSession` — from both the new-session and the disk-attach path — with a
  `SessionToolBinding`, the `cwd` and the session's `SettingsManager`; returning a non-empty list sets
  pi's `noTools: "builtin"` and passes the list as `customTools`. Whole-set because `noTools: "builtin"`
  disables `read`/`write`/`edit`/`bash` together, so a provider that replaced only `bash` would silently
  delete the other three. The `SessionToolBinding` is the answer to a real ordering problem: the ACP
  session id is `session.sessionId`, which does not exist until `createAgentSession` returns, while the
  tools must be built to pass *into* it. The binding is filled the statement after, and a tool can only
  execute inside a prompt — long after that.
- **Allowed deps:** `@earendil-works/pi-coding-agent` (runtime); `@earendil-works/pi-ai` (types + test
  ...
  `ask_user_question` parameter schema); `@thinkrail/contracts` (types only:
  `AskUserQuestion*`/`SlashCommand`/`SkillCatalogEntry`); Node.
- **Forbidden:** `@agentclientprotocol/sdk` and every ACP shape — the sibling `acp` module owns the
  protocol and this one must stay translatable to any other; `@thinkrail/server`, `@thinkrail/shared`,
  `apps/*`; the sibling `acp` module itself (the dependency runs one way, through the injected sink and
  the seams above); `trash` and `@stroncium/procfs` (session-file removal is the injected
  `setSessionFileRemover`, and the OS recycle-bin implementation now lives in
  `packages/server/src/trash`, on the host side of the process boundary — **a consequence of the move:
  with no remover injected, `session/delete` unlinks pi's session file rather than trashing it**);
  Central process/filesystem knowledge — the caller supplies only the desired opaque extension paths for
  a candidate.
