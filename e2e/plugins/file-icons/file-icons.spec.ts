...
test("disabling the plugin falls back to the Remix glyph, and re-enabling restores the material icon", async ({
	page,
}) => {
	await openFixtureProject(page);
	await createWorkspaceViaDialog(page);
	...
	const icon = page
		...
		.getByTestId("file-type-icon");
	await expect(icon).toHaveAttribute("data-icon", "readme");

	await setPluginEnabled(page, "file-icons", false);
	await expect(icon).toHaveAttribute("data-icon", "file");

	await setPluginEnabled(page, "file-icons", true);
	await expect(icon).toHaveAttribute("data-icon", "readme");
});
