...
export interface PluginRuntime {
	...
	allowsExternalFile(workspaceId: string, path: string): boolean;
...
export async function installPlugins(seams: PluginHostSeams): Promise<PluginRuntime> {
	...
	return {
		...
		allowsExternalFile: (workspaceId, path) =>
			registry
				.all()
				.some(
					(entry) =>
						entry.state === "active" &&
						entry.activation?.externalFiles.some((provider) =>
							provider(workspaceId).includes(path),
						),
				),
