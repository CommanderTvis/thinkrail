...
test("a `surfaces:` list pins several barrels to blocks matched by their own subpath label", async () => {
	const root = fixture();
	write(
		root,
		"package/SPEC.md",
...
title: multi-surface
tags: [${PUBLIC_SURFACE_TAG}]
surfaces: [src/index.ts, src/host/index.ts]
---

## Boundary

- **Public surface (\`.\`):** \`fromRoot\`.
- **Public surface (\`./host\`):** \`fromHost\`.
`,
	);
	write(root, "package/src/index.ts", "export const fromRoot = 1;\n");
	write(root, "package/src/host/index.ts", "export const fromHost = 1;\n");

	const result = await run(root);
	expect(result).toMatchObject({ code: 0, stderr: "" });
	expect(result.stdout).toContain("1 enrolled, 2 compared");
});

test("a `surfaces:` list matches blocks by subpath even when the prose order is reversed", async () => {
	const root = fixture();
	write(
		root,
		"package/SPEC.md",
		`---
id: multi-surface-reordered
type: module-design
title: multi-surface-reordered
tags: [${PUBLIC_SURFACE_TAG}]
surfaces: [src/index.ts, src/host/index.ts]
---
...
test("flags a `surfaces:` spec whose block count doesn't match its frontmatter", async () => {
	const root = fixture();
	write(
		root,
		"package/SPEC.md",
		`---
id: surface-count-mismatch
type: module-design
title: surface-count-mismatch
tags: [${PUBLIC_SURFACE_TAG}]
surfaces: [src/index.ts, src/host/index.ts]
---
...
`,
	...
	const result = await run(root);
	expect(result.code).toBe(1);
	expect(result.stderr).toContain("2 `surfaces:` entries but found 1 public-surface block(s)");
});
