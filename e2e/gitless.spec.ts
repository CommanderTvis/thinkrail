...
async function openFolderAndWork(page: Page, stage: () => string | Promise<string>): Promise<void> {
...
async function expectGitToolsWithheld(page: Page, vcs: "none" | "unborn"): Promise<void> {
	...
	// Files, closed the same way, is offered back — proof the menus were read rather than found empty.
	await page.getByTestId("tab-files").first().click({ button: "right" });
	await page.getByRole("menuitem", { name: "Close", exact: true }).click();
...
test("a folder that is not a repository offers neither Changes nor Review", async ({ page }) => {
	await openFolderAndWork(page, () => stagePlainFolder(page));
	await expectGitToolsWithheld(page, "none");
});

test("a folder that is not a repository is offered only itself to work in", async ({ page }) => {
	await openAppFresh(page);
	const dir = await stagePlainFolder(page);
	await page.getByTestId("add-project-menu").click();
	await page.getByTestId("menu-open-project").click();
	await expect(page.getByTestId("project-item").filter({ hasText: basename(dir) })).toBeVisible();
	const start = page.getByTestId("welcome-cta").filter({ hasText: "Start building" });
	await expect(start).toContainText("no worktree to cut");
	await start.click();

	const dialog = page.getByTestId("new-workspace-dialog");
	await expect(dialog).toBeVisible();
	const refused = dialog.getByTestId("ws-target-worktree");
	await expect(refused).toHaveAttribute("data-disabled", "true");
	// The reason is our own tooltip, not the OS's: no `title` for the system to render late and unthemed.
	await expect(refused).not.toHaveAttribute("title", /./);
	await refused.hover();
	await expect(page.getByRole("tooltip")).toContainText("nothing for a worktree to branch from");
	await expect(dialog.getByTestId("ws-target-default")).toHaveAttribute("data-active", "true");
	await expect(dialog).toContainText("not a git repository, so there is nothing to isolate");
