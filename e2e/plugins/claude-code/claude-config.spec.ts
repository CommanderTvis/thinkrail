...
// The setting lives in host config, which outlives a test's state reset — so this spec hands the lane
...
async function openClaudePane(page: Page): Promise<void> {
	...
	const tab = page.getByTestId("tab-plugin:claude-code:config");
	if ((await tab.count()) === 0) {
		await page.getByTestId("side-group-menu").first().click();
		await page.getByTestId("show-tool-plugin:claude-code:config").click();
	}
...
test("the Claude pane reports the account and the usage windows it can name", async ({ page }) => {
	...
	// The stand-in records what it was asked, so the refresh can be checked rather than assumed.
	const refreshMarker = join(E2E_DATA_DIR, "fake-claude-usage-call");
	writeFileSync(
		fake,
		[
			"#!/bin/sh",
			...
			"fi",
			`if [ "$1" = "-p" ]; then`,
			`  echo "$*" > ${JSON.stringify(refreshMarker)}`,
			"  exit 0",
			"fi",
			"",
		].join("\n"),
	...
	// A reading Claude Code has not refreshed says how old it is, rather than looking like a stuck pane.
	writeFileSync(
		join(E2E_HOME_DIR, ".claude", ".claude.json"),
		JSON.stringify({
			cachedUsageUtilization: {
				fetchedAtMs: Date.now() - 50 * 3_600_000,
				utilization: {
					limits: [
						{
							kind: "session",
	...
	// Pressing refresh asks Claude Code itself, which is the only thing that fetches newer numbers.
	rmSync(refreshMarker, { force: true });
	await page.getByTestId("claude-config-refresh").click();
	await expect.poll(() => existsSync(refreshMarker), { timeout: 20_000 }).toBe(true);
	expect(readFileSync(refreshMarker, "utf8")).toContain("/usage");
	await expect(page.getByTestId("claude-usage-age")).toContainText("2d ago");
	// And a window whose reset has already passed says the reading is old, not that it resets now.
	await expect(page.getByTestId("claude-usage-window")).toContainText("Reset since this reading");
...
test("a skill, a hook and a plugin are each composed, then written where the scope says", async ({
	...
	await page.getByTestId("claude-hook-event").click();
	await page.getByTestId("claude-hook-event-PostToolUse").click();
	...
	await expect(hookRow).toContainText("PostToolUse");
...
test("setting source links reveal the nested declaration, including after unsaved edits", async ({
	page,
}) => {
	await openFixtureProject(page);
	await createWorkspaceViaDialog(page);
	const path = join(E2E_HOME_DIR, ".claude", "settings.json");
	mkdirSync(join(E2E_HOME_DIR, ".claude"), { recursive: true });
	const previous = existsSync(path) ? readFileSync(path, "utf8") : null;
	const source = JSON.stringify(
		{ other: { defaultMode: "decoy" }, permissions: { defaultMode: "plan" } },
		null,
		2,
	);
	...
	try {
		...
		await page.getByTestId("claude-surface-settings").click();
		const docs = page
			.locator('[data-testid="claude-setting"][data-key="permissions.defaultMode"]')
			.getByTestId("claude-setting-docs");
		await expect(docs).not.toHaveAttribute("title");
		await docs.hover();
		const tooltip = page.getByRole("tooltip", {
			name: "What this key does, in Claude Code's reference",
		});
		...
		const link = page
			.locator('[data-testid="claude-setting"][data-key="permissions.defaultMode"]')
			.getByTestId("claude-open-source");
		await link.click();
		const editor = page.getByTestId("editor-pane");
		await expect(editor).toContainText('"defaultMode": "plan"');
		await editor.getByRole("textbox", { name: "Editor content", exact: true }).focus();
		await page.keyboard.insertText('"SOURCE_TARGET": null,');
		await expect(editor.locator(".view-line").filter({ hasText: "SOURCE_TARGET" })).toContainText(
			'"defaultMode": "plan"',
		);
		await page.keyboard.insertText("\n\n");
		await expect(editor.locator(".view-line")).toHaveCount(source.split("\n").length + 2);
		await link.click();
		await editor.getByRole("textbox", { name: "Editor content", exact: true }).focus();
		await page.keyboard.insertText("DRAFT_TARGET");
		await expect(editor.locator(".view-line").filter({ hasText: "DRAFT_TARGET" })).toContainText(
			'"defaultMode": "plan"',
		);
...
test("a user setting can be overridden locally without changing its source", async ({ page }) => {
	await openFixtureProject(page);
	const workspace = await createWorkspaceViaDialog(page);
	const userPath = join(E2E_HOME_DIR, ".claude", "settings.json");
	mkdirSync(join(E2E_HOME_DIR, ".claude"), { recursive: true });
	const original = existsSync(userPath) ? readFileSync(userPath, "utf8") : null;
	const source = JSON.stringify({ ...(original ? JSON.parse(original) : {}), model: "sonnet" });
	writeFileSync(userPath, source);
	try {
		await openClaudePane(page);
		await page.getByTestId("claude-surface-settings").click();
		const row = page.locator('[data-testid="claude-setting"][data-key="model"]');
		await row.getByTestId("claude-setting-change").click();
		await expect(page.getByTestId("claude-value-text")).toHaveValue("sonnet");
		await page.getByTestId("claude-value-text").fill("opus");
		await page.getByTestId("claude-value-continue").click();
		await expect(page.getByTestId("claude-edit-scope-user")).toBeVisible();
		await expect(page.getByTestId("claude-edit-scope-project")).toBeVisible();
		await page.getByTestId("claude-edit-scope-local").click();
		await expect(page.getByTestId("claude-edit-scope-local")).toHaveAttribute(
			"aria-pressed",
			"true",
		);
		await expect(page.getByTestId("claude-edit-diff")).toContainText('"model": "opus"');
		const localPath = join(workspace.worktreePath, ".claude", "settings.local.json");
		expect(existsSync(localPath)).toBe(false);
		await page.getByTestId("claude-edit-apply").click();
		await expect(row.getByTestId("claude-setting-override-local")).toHaveCount(0);
		await expect(row.getByTestId("claude-setting-shadowed")).toContainText("sonnet");
		expect(readJson(localPath).model).toBe("opus");
		expect(readFileSync(userPath, "utf8")).toBe(source);
	} finally {
		if (original === null) rmSync(userPath, { force: true });
		else writeFileSync(userPath, original);
	}
});
