...
export default definePluginHost({
	...
	async activate(ctx) {
		...
		const interruptKey = (terminal: { workspaceId: string; tabKey: string }): string =>
		...
		ctx.route(async (request, subpath) => {
			...
			const { session_id: sessionId, cwd, project } = delivery.report;
			if (delivery.status === "running" && sessionId) {
				interrupts.track(
					interruptKey(owner),
					() => agentTranscriptPath(cwd ?? "", sessionId),
					() =>
						publishStatus({
							...push,
							status: "idle",
				...
				interrupts.stop(interruptKey(owner));
			}
		...
		ctx.onTerminal((event) => {
			if (event.kind === "spawned") {
				...
				interrupts.stop(interruptKey(event.terminal));
				forgetStatus(event.terminal.workspaceId, event.terminal.tabKey);
