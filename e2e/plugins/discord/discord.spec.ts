...
import { openFixtureProject, setPluginEnabled } from "../../fixtures/app";

async function openDiscordSettings(page: import("@playwright/test").Page): Promise<void> {
	await setPluginEnabled(page, "discord", true);
...
test.afterEach(async ({ page }) => {
	...
	await setPluginEnabled(page, "discord", false);
});

test("Discord Rich Presence is off by default, wearing ThinkRail's own application once enabled", async ({
	page,
}) => {
	await page.goto("/");
	...
	// Enabling the plugin is the only step: the presence has a name to wear out of the box.
