...
import { CLAUDE_LAUNCH_MENU, claudeLaunchCommand, withLaunchEnv } from "@/lib";
...
export function ClaudeLauncher({ workspaceId, groupId }: { workspaceId: string; groupId: string }) {
	...
	const disableAgentView = useAppStore((state) => state.claudeDisableAgentView);
	const platform = useAppStore((state) => state.hostPlatform);
	const windowsShell = useAppStore((state) => state.terminalWindowsShell);
	...
	const launch = (args?: string) => {
		...
		const started = disableAgentView
			...
			: line;
		useAppStore.getState().addTerminal(workspaceId, started, groupId);
	};
