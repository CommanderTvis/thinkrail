...
- **A client hears about its own workspace and no other, and an agent we did not spawn hears nothing.**
  Every `claude` on the machine dials the same bridge port — it is stamped into every PTY ThinkRail
  spawns, whatever workspace — so one flat client set meant a file opened in one project was announced to
  a session running in another. The CLI's `ide_connected` notification names its own process
  (`params.pid`), which is the only identity a socket ever offers: the host walks that pid up to a pty it
  owns (`terminal`'s `workspaceForProcess`, injected so this module never imports the terminal) and tags
  the socket with that workspace. `selection_changed` and `document_closed` then go only to sockets
  carrying the file's own workspace. A pid belonging to no pty of ours — a `claude` started by hand, or a
  process tree we cannot walk — leaves the socket untagged, and an untagged socket is told about no file
  at all. That is deliberate: it is the same rule, not a special case, and a session outside ThinkRail has
  no workspace whose files are its business.
