...
- **`GitGraph`/`GitGraphCommit`/`GitGraphWorktree`** — moved from `packages/contracts` into `contracts.ts`
  as typebox schemas plus their `Static` projections. The wire method that used to be `git.graph` is now
  `plugin.branch-graph.graph`, params `{ projectId, skip? }`. `git.fetchRemotes` (the branch list's Fetch)
...
- **The Graph panel draws the project's branches, and only reads.** `graph` answers for the project;
  ...
  at that commit's scope. Nothing here mutates a repository.
...
- **Allowed deps:** `@thinkrail/plugin-api` (+ `/host`, `/web`), `@thinkrail/contracts`,
  `@thinkrail/plugin-ui`, `typebox`, `react`.
