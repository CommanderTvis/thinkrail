...
/**
 * The rungs Claude Code's effort slider can draw, in order. Not every environment shows all of
 * them — e.g. `ultracode` only appears behind some flag/tier/model — so the picker is read by
 * whichever subset of this list its labels line actually contains, not by requiring all of it.
 */
...
/** Below this many labels found on a line, it's too little to trust as the picker's track. */
const MIN_LABELS = 2;
...
export function effortHighlight(lines: readonly string[]): ClaudeEffortLevel | undefined {
	for (let i = lines.length - 1; i >= 0; i--) {
		...
		if (centers.size < MIN_LABELS) continue;
