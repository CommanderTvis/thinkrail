...
export function createClaudeTerminalFacts(ctx: PluginWebContext<typeof claudeCodeContract>) {
	...
	return function ClaudeTerminalFacts({ terminal }: { terminal: TerminalAccessoryApi }) {
		...
		const ideContextEnabled = useClaudeCodeStore((s) => s.ideContextByTerminal[ideContextKey]);
		const setIdeContext = useClaudeCodeStore((s) => s.setIdeContext);
		useEffect(() => {
			if (agent?.kind === "claude" && ideContextEnabled === undefined)
				setIdeContext(terminal.workspaceId, terminal.tabKey, true);
		}, [agent?.kind, ideContextEnabled, setIdeContext, terminal]);
		...
		const toggleIdeContext = () => {
			const next = !(ideContextEnabled ?? true);
		...
		return (
			<>
				...
				>
					...
					{state?.usage ? <TerminalUsageChip agent="Claude" usage={state.usage} /> : null}
					{state?.todos?.length ? <TerminalPlan agent="Claude" todos={state.todos} /> : null}
					<TerminalIdeContextChip enabled={ideContextEnabled ?? true} onToggle={toggleIdeContext} />
					<TerminalAttachButton
