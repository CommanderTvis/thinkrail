...
function fakePicker(initialHighlight: number): FakePicker {
	...
	return {
		...
		io: {
			...
			// Claude's faint prompt suggestion sits in an idle composer; only a faint-blind read sees it.
			readLines: (options) =>
				confirming
					? CONFIRM
					: open
						? renderPicker(highlighted)
						: [options?.omitFaint ? "❯" : "❯ fix the Composer error in the images commit too"],
