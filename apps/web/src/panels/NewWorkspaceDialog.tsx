...
import { CLAUDE_MODELS, claudeLaunchCommand, cn, shellQuotePath, withLaunchEnv } from "@/lib";
import {
...
export function NewWorkspaceDialog({
	...
	const create = async () => {
		...
		if (agent === "claude") {
			...
			const line = claudeLaunchCommand(claudeCommand, args);
			store.addTerminal(
				workspace.id,
				store.claudeDisableAgentView
					? withLaunchEnv(line, {
							platform: store.hostPlatform ?? undefined,
							windowsShell: store.terminalWindowsShell,
						})
					: line,
				centre,
				"center",
