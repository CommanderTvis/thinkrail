...
beforeAll(async () => {
	...
	setSessionEventSink((sessionId, event) => {
...
test("agent_settled carries the final attempt's terminal metadata", async () => {
	...
	const session = await createSession({
		cwd,
		model: toEngineModel(fauxA.getModel()),
	});
	...
	const hydrated = await getSessionMessages(session.sessionId, cwd);
...
test("thinkingLevels is pi's per-model support truth, not a reasoning boolean widened to all seven", () => {
	...
	expect(toEngineModel(reasoner).thinkingLevels).toEqual([
		"off",
		"minimal",
		"low",
		"medium",
	...
	expect(toEngineModel(alwaysThinks).thinkingLevels).not.toContain("off");
});
...
test("createSession rejects an unknown/unavailable model ref (no arbitrary baseUrl injection)", async () => {
	...
	await expect(createSession({ cwd: tmpCwd("trpi-bad-"), model: bogus })).rejects.toThrow(
		/Unknown or unavailable model/,
	);
});
...
test("listSessions reports a workspace's live sessions; getSessionMessages returns its transcript", async () => {
	...
	expect(live?.cwd).toBe(resolve(cwd));
	expect(live?.live).toBe(true);
	expect(await listSessions(tmpCwd("trpi-hyd-other-"))).toHaveLength(0);

	const { messages } = await getSessionMessages(s.sessionId, cwd);
...
test("listSessions ignores a live session's transient physical rewrite but stays strict for detached files", async () => {
	...
	try {
		...
		expect((await listSessions(cwd)).map((row) => row.sessionId)).toContain(s.sessionId);

		removeSession(s.sessionId);
		await expect(listSessions(cwd)).rejects.toThrow("unreadable or malformed");
	} finally {
		setSessionManagerFactory(() => SessionManager.inMemory());
	}
});
...
test("disk-reopen: a disposed session is re-listed from disk and re-opened with its transcript (restart survival)", async () => {
	...
	try {
		...
		const fromDisk = (await listSessions(cwd)).find((x) => x.sessionId === s.sessionId);
		expect(fromDisk).toBeDefined();
		expect(fromDisk?.live).toBe(false);

		const otherCwd = tmpCwd("trpi-other-");
		expect((await listSessions(otherCwd)).map((x) => x.sessionId)).not.toContain(s.sessionId);

		const { summary, messages } = await getSessionMessages(s.sessionId, cwd);
		expect(summary.live).toBe(true);
		expect(messages.some((m) => m.role === "user")).toBe(true);
		removeSession(s.sessionId);

		const [a, b] = await Promise.all([
			getSessionMessages(s.sessionId, cwd),
			getSessionMessages(s.sessionId, cwd),
		]);
		expect(a.summary.live && b.summary.live).toBe(true);
		expect((await listSessions(cwd)).filter((x) => x.sessionId === s.sessionId)).toHaveLength(1);
		removeSession(s.sessionId);
	} finally {
		setSessionManagerFactory(() => SessionManager.inMemory());
	}
...
test("deleteSession removes an empty live chat whose reserved transcript path is not materialized", async () => {
	...
	try {
		...
		await deleteSession(session.sessionId, cwd);
...
test("deleteSession tombstones its id so a stale transcript cannot reattach in this host", async () => {
	setSessionManagerFactory((cwd) => SessionManager.create(cwd));
	setSessionFileRemover(async (input) => {
	...
	try {
		...
		await expect(getSessionMessages(session.sessionId, cwd)).rejects.toThrow(
...
test("a malformed detached transcript is never treated as authoritative absence", async () => {
	...
	let trashCalls = 0;
	setSessionDeletedSink((sessionId) => published.push(sessionId));
	setSessionFileRemover(async () => {
		trashCalls++;
	});
	let sessionId: string | undefined;
	try {
		...
		await expect(listSessions(cwd)).rejects.toThrow("unreadable or malformed");
		await expect(deleteSession(session.sessionId, cwd)).rejects.toThrow("unreadable or malformed");
		expect(trashCalls).toBe(0);
		expect(published).toEqual([]);
		expect(existsSync(info.path)).toBe(true);

		writeFileSync(info.path, transcript);
		const restored = await getSessionMessages(session.sessionId, cwd);
...
test("a pending delete blocks live commands, then trash failure restores the same runtime", async () => {
	...
	try {
		...
		deleting = deleteSession(session.sessionId, cwd);
		await trashStarted;
		expect(hasSession(session.sessionId)).toBe(false);
...
test("concurrent deletes of one chat coalesce into a single owned transaction", async () => {
	...
	try {
		...
		first = deleteSession(session.sessionId, cwd);
		second = deleteSession(session.sessionId, cwd);
		await trashStarted;
...
test("archival teardown is not blocked by a chat whose recoverable delete is mid-trash", async () => {
	...
	try {
		...
		deleting = deleteSession(doomed.sessionId, cwd);
		await trashStarted;

		await removeCwdSessions(cwd, true);
...
test("ensureSessionAttached: a detached-but-persisted session comes back live; a missing id is `false`", async () => {
	...
	try {
		...
		expect(await ensureSessionAttached(s.sessionId, cwd)).toBe(true);
		expect(hasSession(s.sessionId)).toBe(true);
		expect(await ensureSessionAttached(s.sessionId, cwd)).toBe(true);

		expect(await ensureSessionAttached("no-such-session", cwd)).toBe(false);
		removeSession(s.sessionId);
	} finally {
		setSessionManagerFactory(() => SessionManager.inMemory());
	}
...
test("removeWorkspaceSessions: archives a workspace's live sessions + purges their on-disk transcripts, leaving siblings", async () => {
	...
	try {
		...
		expect(await listSessions(doomedCwd)).toHaveLength(1);
		expect(await listSessions(keepCwd)).toHaveLength(1);

		await removeCwdSessions(doomedCwd, true);

		expect(hasSession(doomed.sessionId)).toBe(false);
		expect(await listSessions(doomedCwd)).toHaveLength(0);
...
test("extension-UI bridge: confirm round-trips, a declined ask resolves undefined, dispose cancels", async () => {
	const asked: { sessionId: string; dialog: ExtUiDialog }[] = [];
	const pending: {
		resolve: (value: string | boolean | null) => void;
		signal: AbortSignal;
	}[] = [];
	setExtUiBridge({
		ask: (sessionId, dialog, signal) => {
			asked.push({ sessionId, dialog });
	...
	const answerLast = (value: string | boolean | null): void => {
		const last = pending.at(-1);
		if (!last) throw new Error("expected the bridge to have been asked");
		last.resolve(value);
	};
	...
	expect(asked.at(-1)?.dialog.kind).toBe("confirm");
	expect(asked.at(-1)?.sessionId).toBe("sess-extui");
	answerLast(true);
