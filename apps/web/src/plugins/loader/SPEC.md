...
- **`openTerminal` on a known tabKey attaches instead of no-oping.** `addTerminal` (the store action that
  ...
  `terminalsByWorkspace` first: an unknown `tabKey` goes through `addTerminal` as before; a known one queues
  a placement intent using the catalogued title and selects the terminal without changing its metadata.
  Both paths target the requested or last-focused center group: plugin-opened terminals are main-column
  resources unless their caller names a different center group. The terminal catalog is a resource list,
  not proof that a terminal still has a visible layout tab.
  `openChat`'s `prompt` option is submitted (`session.prompt`, with the same optimistic
