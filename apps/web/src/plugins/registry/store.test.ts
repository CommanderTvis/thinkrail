...
describe("asset icons", () => {
	it("draw one stable component per asset, and fall back when the plugin declares no assets", () => {
		const base = pluginAssetBase(roster({ id: "codex", assets: "assets" }), "http://host");
		expect(base).toBe("http://host/plugin/codex/assets");
		const icon = pluginIcon("asset:codex.svg", false, base);
		expect(pluginIcon("asset:codex.svg", true, base)).toBe(icon);
		expect(icon).not.toBe(pluginIcon("asset:codex.svg"));
		expect(pluginAssetBase(roster(), "http://host")).toBeUndefined();
		expect(pluginIcon("asset:codex.svg")).toBe(pluginIcon("no-such-icon"));
	});
});
