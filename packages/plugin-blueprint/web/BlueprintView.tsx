	...
	function useBlueprintState(workspaceId: string): BlueprintState | null | undefined {
		useEffect(() => {
			...
			void ctx
				...
				.then((result) => useBlueprintStore.getState().setState(workspaceId, result.state))
				.catch(() => {});
			...
			>
				<header className="flex items-center gap-8 border-b border-border-default px-16 py-8">
					...
					<Button
						...
						onClick={() => void ctx.editors.open(state.workspaceId, BLUEPRINT_FILE, { raw: true })}
					>
						Open raw source
					</Button>
		...
		if (state === undefined) {
			return (
				<div className="flex h-full items-center justify-center">
					<Loader2 className="size-20 animate-spin text-text-muted" />
				</div>
			);
		}
