...
import { BLUEPRINT_APPENDIX, openingPrompt } from "./prompts";
import {
	blueprintBrief,
	closeBlueprint,
...
export default definePluginHost({
	...
	activate(ctx) {
		...
		ctx.method("open", (params) => {
			...
			const source = resolveBlueprintSource(workspace.worktreePath, params.source);
			...
			return { state, opening: openingPrompt(source), systemPrompt: BLUEPRINT_APPENDIX };
		});
