...
import { CODEX_MODELS, type CodexLaunchOptions, codexLaunchLine } from "../launch";
import { CODEX_ID } from "../manifest";
import { CodexGlyph, createCodexLauncher } from "./CodexLauncher";
...
export default definePluginWeb<typeof codexContract>({
	activate(ctx) {
		...
		ctx.workspaceAction({
			id: "codex-launch",
			component: createCodexLauncher((workspaceId, groupId, preset) => {
				void ctx.openTerminal(workspaceId, { command: launchLine(ctx, { preset }), groupId });
			}),
		});
		...
		const CodexTerminalFacts = createCodexTerminalFacts(ctx);
