...
import { CLAUDE_PLUGIN_SCOPE_WORDING, CLAUDE_WRITABLE_SCOPES } from "../webValues";
...
function toggleFor(item: ClaudeCapability): { edit: ClaudeEdit; title: string } | null {
	...
	if (item.kind === "mcp" && item.origin.path === null) return null;
	if (item.kind === "mcp") {
		return {
							...
							return (
								...
								>
									<div className="flex min-w-0 items-center gap-4">
										...
										{item.origin.path ? null : <ScopeChip scope={item.origin.scope} />}
										{item.enabled ? null : (
											<span
												title={
