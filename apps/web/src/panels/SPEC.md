...
branch tip a line rising out of it into nothing. **Every turn is the same quarter of the same
circle.** A curve that bent across whatever horizontal distance it had to cover made a fan of merges
splay — seven lines into one dot, each at a different curvature — and made an arriving line and a leaving
one look like different shapes. A line now runs in its lane, turns through one fixed radius, and runs
straight to where it is going, so the two are mirror images and a fan reads as a comb. A merge draws the lane it opens as well: its
parents after the first each claim a lane, and without a curve from the dot into that lane a branch
simply appeared below the merge, attached to nothing. An orphan branch needs no special case and gets
none — it shares no ancestry, so it claims a free lane, ends at its own root, and frees the lane there;
`graphLanes.test.ts` and `e2e/git-graph.spec.ts` both pin that. The shared width is the widest row *in view*, not in the
...
kick-off tail is identical in both modes. **The agent is a choice too**: a chip pair (`ws-agent`, the
shared `chips.ts` look the blueprint dialog uses — *Bundled agent* / *Claude Code*, the latter disabled
with "Turn Claude Code on in Settings." as its title until the setting is on) sits where the model
pickers start. With Claude Code chosen the pi model and effort pickers give way to one Claude model menu
(`ws-claude-model`: Default model or a `CLAUDE_MODELS` alias, sent as `--model`), and create opens **a
terminal in the centre group** running `claudeLaunchCommand(claudeCommand, …)` with the prompt as the
CLI's positional argument (single-quoted through `shellQuotePath`; an empty prompt starts a bare
session) instead of a chat — the same launch the tab strip's launcher performs, seeded. No pi session
...
a project picker, the prompt hero, and the reused
  ...
  **Edit** action; a global template has
  ...
  the boolean ever crosses the wire, see `submodule-server-analytics`. **`ClaudeCodeSettings`** carries the integration switch and the
...
- **Allowed deps:** `store`, `transport`, `components` (`SkeletonRows` — every async panel's pending
  ...
  `@xterm/*`, `pdfjs-dist`) loaded via `import()`.
...
- **Paths are absolutized by the host.** The client addresses files worktree-relative; Claude Code
  expects a path it can open, so `ideBridge.selectionChanged`/`documentClosed` join the workspace's
  worktree path server-side. An `external-file` path is already absolute and passes through.

## Claude Code's IDE actions

`ideActions.ts` answers what a `claude` CLI asks of this editor through the host's bridge
(`packages/server/src/ideBridge/SPEC.md`): open a file, list open editors, close a tab. It is registered
from `main.tsx` through `transport`'s `setIdeActionHandler` seam, since `transport` may not import
`panels`.

- **Every action replies, including a failed one.** A thrown error becomes the CLI's tool error rather
  than a hang — the bridge's timeout is a backstop, not the normal failure path.
- **Paths arrive absolute and are made worktree-relative** through the same `projectRelativePath` every
  other open goes through, so an IDE-driven open and a user-driven one produce the identical tab identity
...
A Claude Code terminal shows two chips under it: the model, and the reasoning effort. They come over the
same agent status protocol the badge already rides — a report the plugin POSTs to the host, which pushes
it here (`contracts/agentStatus.ts`) — because the answer
belongs to the session and can change inside it — `/model` mid-chat moves it, and so does a different
effort on the next turn.

- **Per terminal, never per app.** Two Claude sessions in two tabs are two chats, each on whatever it was
  last told to use, so the state is keyed by terminal (`claudeCodeByTerminal`) exactly like the status.
- **The last reported answer stands.** Only some events carry these fields; a `tool_complete` that omits
  them means "nothing new to say", not "no longer known", so the store keeps the previous values rather
  than blanking the chips between turns.
- **A report can be facts without a status.** `/model` changes what the session runs on without changing
  what it is doing, so the switch arrives with `status: null` and the badge keeps whatever it had. Before
  a first status there is nothing to keep, so such a report is dropped rather than inventing one.
- **The chips are the pane's, not the terminal program's.** They sit in the pane below the xterm host,
  which shrinks by their height, so they never overlap what the agent is drawing — Claude Code has its own
  footer and this is not competing with it.
- **The model chip is also the switch — and the switch is session-only.** Clicking it opens a menu of
  the CLI's model aliases (`lib.CLAUDE_MODELS`, the same list the launcher's context menu builds its
  `--model` presets from) and a pick drives the session's own interactive `/model` picker
  (`lib.driveModelPicker` over `terminal.write` + the xterm buffer): open it, arrow the `❯` highlight
  ...
  keystroke string. If no picker renders (~3.6s) or no row ever names the pick, it presses Esc and
  says so in a toast. It types where the attach chip already types, with the same assumption: the
  session is at its prompt. The chip then updates the way it always has — from the plugin's
  `model_switch` report, not from the click — so what it shows stays what the session confirmed, never
  what the menu hoped.
- **A drive owns the pane until it ends.** The driver is a feedback loop typing into a live TUI and
  reading what comes back, so anything else the user does lands *in the middle of it*: a keystroke is
  interleaved with the picker's own, a click on the chip starts a second drive, a mouse report inside a
  mouse-tracking TUI moves the highlight the driver is counting on. So while one runs, the pane is
  sealed — an overlay covers the terminal and its chips, and `onData`, the single funnel every keystroke,
  paste and mouse report passes through on its way to the pty, drops what it is given. The seal is lifted
  however the drive ended, including by a rejected transport call: a seal that outlived its drive would
  leave the terminal permanently unusable, which is worse than the interleaving it prevents. Sealing is
  not the same as the single-flight guard it stands next to — the guard keeps a second *drive* out, and
  cannot see a keystroke.
- **The effort chip is a switch too**, and the same one: `lib.driveEffortPicker` steers `/effort` to the
  chosen rung and takes it with `s`, session-only, sharing the model chip's driver, its single-flight
  guard, and its toasts. What differs is the picker — a slider read by its marker rather than a list read
  by its highlight — which is `lib`'s problem, not this module's. One thing does differ on this side:
  no plugin event reports an effort change (there is no `effort_switch`), so a drive that ends in
  `switched` writes the chosen rung into the session's facts itself. Without that the chip would keep
  the old level until the next report happened to carry a new one — which the user reads as "the CLI
  changed, the chip didn't". The model chip keeps waiting for `model_switch`, because that report exists.
- A model id is mostly prefix, so the chip shows the part that identifies it (`opus-5`) with the full id
  on hover. Effort is shown in the agent's own words rather than translated into a scale of ours.
...
- **Claude's own plan lives under Claude's terminal.** `TodoWrite` rewrites the whole plan, so the
  ...
  facts: a report that says nothing about todos leaves the last plan standing, a rewrite replaces it
  whole. The row grows a `n/m` chip (`terminal-plan-toggle`) that toggles an overlay checklist
  (`terminal-plan`) floating above the fact row — an overlay, not a split, so opening it never resizes
  the PTY. Items render pending / in-progress / completed with the in-progress item speaking in its
  `activeForm`, the same present-tense voice Claude Code's own spinner uses. This deliberately does
  **not** touch the Plan pane: that pane is pi's, wired to `todo.*` state on a chat session, and a
  second writer keyed by terminal would blur whose plan it shows — many Claude terminals can run at
  once, and each carries its own list where it runs.
- **A third chip says where the agent started.** Every report carries Claude's own `cwd` — the directory
  `claude` was launched in, which a session cannot leave, and which is not the worktree root when the user
  `cd`'d first. It is shown **whole**, with `$HOME` as `~`, exactly as the agent's own header prints it:
  the first version showed it relative to the worktree, and `applications/axel-springer` on a chip reads
  as a path to nowhere. A long one loses its middle rather than its end (`~/…/2026/spring`), since where a
  path starts and what directory it actually is are the identifying halves; the whole of it is on hover.
  The chip is lowercase and carries a folder mark, because a path in the pill's small caps
  (`APPLICATIONS/AXEL-SPRINGER`) is not a path anyone recognises.

### Attaching a file to what Claude is about to be asked

`TerminalAttachFile` browses the worktree (`fs.readDir`, folders first) and types **`@<path> `** into the
terminal — Claude Code's own syntax for putting a file, or a directory's listing, in front of the agent.
It types and stops there: no newline, because the user is mid-sentence and the prompt is theirs to send.
The worktree is where the agent works, not the limit of what it may be shown: **`elsewhere…` hands over to
the host's own file picker** (`dialog.selectFile`, the one the launch-command field and "Open project"
use), so a file anywhere on the machine can be attached. That picker opens on the *host*, which is why it
is the escape hatch and not the primary way in — the browser is the client, and the client may not be
sitting at that machine.

The path is written for **the directory the agent reported**, not the worktree: relative when the file is
under that directory, absolute otherwise — including when no `cwd` has been reported at all. Claude takes
either ("File paths can be relative or absolute"), and an absolute path cannot be misread as relative to
somewhere else, which is exactly what a `cd`'d session would do to a worktree-relative one. A directory is
attached from its own header row (`attach this folder`), since clicking a folder in the list means "go
in".

## The Claude configuration pane

The pane is four surfaces — Context, Settings, Capabilities over one snapshot, and Account, which reads
its own — **and the pane itself never scrolls sideways**: the switcher is a one-row scroller of its own
(hidden scrollbar, the same treatment the tab strips get) so four segments plus the refresh control cannot
widen the pane, and the root clips what a long value would otherwise push out. A rail panel that scrolls
horizontally hides its own right edge, which is where every row's controls live. Pinned by
`e2e/claude-config.spec.ts` — and each lives in its
own file (the context surface stays in `ClaudeConfigPanel`, the others are `ClaudeSettingsSurface`,
`ClaudeCapabilitiesSurface` and `ClaudeAccountSurface`), sharing row furniture through `ClaudeConfigParts` (scope chip, source
link, row action, the open-source hook). `ClaudeConfigPanel` owns the snapshot, the surface switch and
the one pending edit; the surfaces never talk to the transport. The header's refresh re-reads the
**account and its usage windows** as well as the configuration: the percentages are the reason anyone
presses it, and they come from Claude Code's own cache, which the button has no other way to re-read.
The surfaces watch one revision counter — the workspace's file-change tick plus the manual presses, both
of which only climb — rather than each growing its own refresh path.

**Refresh asks Claude Code for the numbers; it does not just re-read them.** The percentages live in a
cache Claude Code writes, and re-reading that file shows whatever it last fetched — on a quiet day, days
old. `claude -p "/usage"` runs the same slash command a session would and makes it fetch again, so the
press does what a person means by it. It runs **only on the press**: a file-change tick re-reads the
cache, because spawning the CLI on every write in the worktree is not a refresh, it is a tax. The pane
says which it is doing while it waits, and gives the reading's age in words next to its timestamp. A
window whose reset has already passed says *Reset since this reading* rather than *Resets now*: with an
old reading, "now" is a claim about the service the data cannot support.

**An `@`-import is drawn as a branch of the file that pulled it in.** Indentation alone read as a
near-invisible step — a row simply further right, with nothing saying *why* — so each level draws its
own vertical rule and the innermost one carries an **elbow** whose arm reaches into the row it belongs
to, the shape every file tree uses. A last child stops its rule at the elbow (`└`) instead of running it
past the end of the branch (`├`), which is what makes a nested group read as finished rather than
continuing into whatever row happens to follow. Guides stop at four levels deep; beyond that the
indentation carries it alone. **The rows carry no rule of their own**: a line under every row crossed
the branch it was supposed to be beside, so the elbow appeared to grow out of the separator above it
rather than out of its parent. The tree is the structure; a grid drawn over it is one structure too
many.

**A capability row's actions live in its ⋮ menu, and the row itself only states facts.** Rows are
grouped under one heading per kind (`claude-capability-section`, in the fixed order MCP servers, Plugins,
Skills, Subagents, Hooks, Marketplaces). A kind Claude Code gives no switch (subagents, hooks,
marketplaces) simply has no `Turn on` / `Turn off` in its ⋮ menu — an earlier "no switch" marker, first
per row and then per heading, was noise that named an absence nobody asked about. A row is
the name on its own line — the full pane width, with the scope chip, `off · user` and the ⋮ at its right
— over its source path and detail; `Turn on` / `Turn off` and `Uninstall…` sit behind the **⋮ menu**. A
per-row kind gutter and inline action buttons were both tried and squeezed the name out of narrow panes
(kind + name + scope + `off · user` + `no switch` + kebab left `claude-co…` and `MARKETPLACE` overrunning
the path beneath), and a destructive action one pixel from a switch people flip casually is a mis-click
waiting to happen.

**A plugin row can also be uninstalled, and that runs a command.** Turning a plugin off writes a settings
key like any other capability; removing one is Claude Code's own `claude plugin uninstall`, because the
plugin exists in three places the pane does not own ([[submodule-server-claude-config]]). Uninstall
appears only on rows whose scope Claude's CLI accepts
(`user` / `project` / `local`; a managed setting has no such scope, so the item is absent). Choosing it
opens `ClaudePluginUninstallDialog`, which shows **the exact argv the host will run** and nothing else to
decide: the host composes that line and runs that same line, so it is a preview, not a description of one.
That is the command-shaped twin of the diff `ClaudeEditDialog` shows — every change this pane makes to
someone's machine is seen, in its own vocabulary, before it happens.

**Every change is two dialogs, and they ask different questions.** A dialog composes *what* to write —
`ClaudeValueDialog` for a settings value, `ClaudeMcpServerDialog` for a server — and hands a complete
`ClaudeEdit` to `ClaudeEditDialog`, which asks *where* it goes and shows the resulting diff to approve.
Merging them would re-plan a diff on every keystroke of a half-typed value, and would blur the one
moment that matters: the diff the user says yes to is the change that lands. Both composing dialogs are
**remounted per invocation** (a `key`), because they seed their fields from the value they were opened
on; carrying that state into the next row would edit one key with another's value.

- **The value is the control.** Clicking the rendered value opens the editor for it (an *Edit* tooltip on
  hover says so); a separate Change button sat away from the thing it changed, which is where the reader's
  eye already is. **Its current type chooses that editor**: a boolean gets a switch, a number an input, a
  list a line-per-entry textarea. A value that is none of those (a nested object) is not a control at all —
  the row says *edit as a file* and links to it, rather than offering something that would flatten it
  silently. Remove stays a row action: it is destructive, and destructive actions do not hide under the
  thing they destroy.
- **Servers Claude reaches beyond the files are rows too, without a switch.** After the snapshot the
  pane asks `claudeConfig.mcpList` — claude.ai connectors and plugin servers, as Claude's own CLI reports
  them with their health — and appends those rows to the MCP section (`ClaudeConfigPanel`'s `reached`,
  a second request so the health check never delays the files). A row whose origin has no path is one
  of these: its ⋮ carries no Turn on/off, because the switch is `/mcp` inside Claude, and a connector
  disabled there for this project says `off · local` and links the `disabledMcpServers` list in
  `~/.claude.json`. `e2e/claude-config.spec.ts` drives it against a stand-in `claude`.
- **Account is who Claude Code is signed in as, and how much of the plan is spent.** `claudeConfig.account`
  answers with `claude auth status --json` (email, plan, organization) beside the rate-limit windows
  Claude Code already caches in `~/.claude.json` under `cachedUsageUtilization` — so the pane costs no
  network call of its own and shows what the agent itself last saw, dated. **Only windows that can be
  read from the **`limits` list Claude Code caches beside its per-key buckets**: every entry there carries
  its own kind, severity, and — for a model-scoped week — the model's display name, so a limit is labelled
  with the model it belongs to (`Fable limit`) instead of the rotating code name the buckets are keyed by
  (`nimbus_quill`, `cinder_cove`). The buckets remain a fallback for a cache written before that list
  existed, and there only the two windows that can be named truthfully are drawn. The bar wears the
  severity Claude Code already decided on rather than forming a second opinion from the percentage.
  Neither half is fatal to the other: a `claude` that cannot answer still leaves the cached usage on
  screen, and a machine with no cached reading still shows the account.
- **A capability row's switch is the honest one for its kind:** deny/allow for a server, `enabledPlugins`
  for a plugin, `skillOverrides` for a skill. A subagent row offers none, because Claude Code has
  none. When something is off, the row names the scope that turned it off and links to that file too —
  "off" without "by whom" is the question the pane exists to answer.
- **Adding is a first-class action on both surfaces**, not a hidden affordance: a key that appears in no
  file yet (with the documented names as completions), and — from the capabilities header — an MCP
  server, a skill, a hook or a plugin. Each is its own compose dialog, and each hands a finished
  `ClaudeEdit` to the same approval step; the field furniture is shared (`ClaudeConfigParts`) so four
  forms cannot drift into four looks. The dialogs say plainly what they are about to grant: a hook and a
  plugin both run commands with the user's permissions, and a secret typed into a server form will appear
  in the diff.

Each row's path is a `SourceButton`, and clicking one opens that file as an ordinary
tab — `external-file` when the path escaped the worktree, which is most of Claude's configuration.

- **A link lands on the entry, not the top of the file.** `~/.claude.json` holds every project's MCP
  servers at once and a settings file holds dozens of keys, so opening at line 1 leaves the reader
  hunting for the row they just clicked.
- **The host sends a key path; the line is computed here.** `ClaudeConfigOrigin.keyPath` names the value
  as JSON object keys (`["mcpServers", "git"]`), and `FilePane` turns it into a line with `jsonKeyLine`
  against `tab.content` — the text the editor is about to show. A line resolved host-side would be
...
- **PDF tabs render bytes over their own route, not tab content.** `FilePane` gates on `lib.isPdfPath`
  (restricted to `kind === "file"` — an external-scope PDF is out of scope for v1, since `claudeConfig`'s
  configuration is always JSON/markdown) and shows lazy `PdfPreview`, which fetches from
  `filesUrl.worktreeFileUrl(workspaceId, path)` — the same worktree-scoped `/files/…` HTTP route
  `markdownLinks`' `DocumentImage` already streams raw image bytes through. `serveWorktreeFile`
  (`packages/server/src/host/server.ts`) needed no change, since it already serves arbitrary bytes with
  ...
  more beside the PDF, and every one of them re-fetched and re-rasterized a document that had not
  changed. That is the whole point for a document that is *generated*: LaTeX, a diagram export, a
...
- **We render the PDF ourselves (`pdfjs-dist`) rather than handing it to an `<iframe>`.** The first
  ...
  viewer's own document never reach this page, and the plugin is not scriptable, so there is no seam to
  hook. Obsidian reaches the same conclusion and ships PDF.js for its own viewer. Owning the render
  surface is what buys pinch-zoom, and later makes in-app search and an outline possible.
- **Each page is re-rasterized per scale, at device pixel ratio.** Scaling a finished bitmap is what the
  iframe already did badly; `PdfPage` instead re-renders through pdf.js at `scale × devicePixelRatio` and
  presents at CSS size, so text is sharp at 600% and on a retina display. A render in flight is
  **cancelled, not awaited** (`RenderTask.cancel()`): a pinch emits scales far faster than a page
  rasterizes, and pdf.js rejects a second render onto a canvas the first still owns.
- **The canvas is a picture; the text is a layer over it.** A rasterized page has no words, so nothing in
  it could be selected or copied — a PDF with a perfectly good text layer read as an image. `pdfEngine`'s
  `renderTextLayer` puts pdf.js's own `TextLayer` (transparent, absolutely positioned spans) over each
  canvas, at the **CSS** scale rather than the device-pixel one the canvas rasterizes at, and cancels it
  alongside the render when the scale moves again. Its geometry comes from three custom properties pdf.js
  writes per span and multiplies by a `--total-scale-factor` the container carries, so
  `pdfTextLayer.css` holds exactly those rules, copied from `pdfjs-dist/web/pdf_viewer.css` rather than
  imported — that file is the entire viewer, 160KB of toolbar, sidebar and `:root` palettes we do not
...
- **The zoom follows the size of the gesture.** A pinch is a stream of small deltas, and stepping by a
  ...
  (`exp(-delta / sensitivity)`), so a pinch moves it a percent or two per tick, and the delta is bounded
  first so a *mouse* notch — a hundred pixels, or three lines, or one page, depending on the device —
  stays one ordinary step rather than a jump to the limit.
- **What the gesture moves and what gets redrawn are separated.** Rasterizing on every wheel event is the
  other half of "not smooth": a pinch asks for dozens of full renders a second and gets a stutter. The
  live scale is presented by stretching the pixels already on the canvas — the page's box takes the new
  size and an inner element carries `scale(live / rasterized)` — and the pages are re-rendered sharp once
  the scale has held still for `PDF_RASTER_SETTLE_MS`. The text layer rides the same transform, so
  selection stays over its glyphs while the gesture is in flight.
- **The worker is an emitted asset, not a bare specifier.** `pdfEngine.ts` sets `workerSrc` from
  `pdfjs-dist/build/pdf.worker.min.mjs?url`, so Vite emits it and rewrites the reference to the built
  path. This matters for the desktop app specifically, which serves the built bundle from disk where an
  unresolved specifier would fail silently — the same bundling hazard `apps/desktop/SPEC.md` documents
  for pi's dynamic imports. Verified by checking the asset lands in `dist/assets/` and the reference in
  the lazy `PdfPreview` chunk points at it.
