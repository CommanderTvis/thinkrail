...
const DETECTED_AGENTS: readonly string[] = ["codex"];
...
export interface AgentWatch {
	agentFor(workspaceId: string, tabKey: string): string | undefined;
	poke(): void;
...
export function createAgentWatch(deps: AgentWatchDeps): AgentWatch {
	...
	return {
		...
		// Arms the timer only; sweeping inline would block attach.
		poke() {
