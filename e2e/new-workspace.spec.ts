...
test("the dialog lists local branches (no stray origin) and creates a worktree", async ({
	...
	// Chrome, not content: it explains the workspace rather than offering a string to take away.
	await expect(page.getByTestId("scope-base")).toHaveCSS("user-select", "none");
