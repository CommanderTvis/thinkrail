...
export interface BlueprintStore {
	read(): PersistedBlueprints;
	write(value: PersistedBlueprints): void;
}

function inMemoryStore(): BlueprintStore {
	let value: PersistedBlueprints = {};
	return {
...
let store: BlueprintStore = inMemoryStore();

export function setBlueprintStore(next: BlueprintStore): void {
	store = next;
}
...
function restore(workspaceId: string, worktreePath: string): Session | null {
	const record = store.read()[workspaceId];
...
function stateOf(session: Session, doc: BlueprintDoc | null): BlueprintState {
	const title = doc?.frontmatter ? titleFromFrontmatter(doc.frontmatter) : null;
	return {
		workspaceId: session.workspaceId,
		source: session.source,
		brief: title ?? describeSource(session.source),
		agentId: session.agentId,
...
function publishAndRemember(session: Session, doc: BlueprintDoc | null): BlueprintState {
	...
	publish({ workspaceId: session.workspaceId, state: structuredClone(state) });
	return state;
}
 ...
 * told, delivered host-side by the caller — see host/index.ts, H10/H11.
 */
...
/** Clears live sessions only — the backing store survives, the way a real restart leaves the record. */
