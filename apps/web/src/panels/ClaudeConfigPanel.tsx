...
import { ClaudeAccountSurface } from "./ClaudeAccountSurface";
...
// An `@`-import nests under the file that pulled it in — see claudeConfig/SPEC.md for how it is drawn.
						...
						{indentGuides(layer).map((guide, level, all) =>
							level === all.length - 1 ? (
								<span
									...
									className={`relative ml-sm w-md shrink-0 after:absolute after:top-1/2 after:-right-sm after:left-0 after:h-px after:bg-border-default after:content-[''] ${
										last
...
export function ClaudeConfigPanel({ workspaceId }: { workspaceId: string }) {
	...
	// Both counters only ever climb, so their sum is a revision the account surface can watch.
		...
		(generation: number) => {
			...
			void getTransport()
				...
				.then((result) => {
					...
					setError(null);
					void getTransport()
						.request("claudeConfig.mcpList", { workspaceId })
						.then((listed) => {
	...
	return (
		...
		>
			<div className="flex h-panel-header-row shrink-0 items-center gap-sm px-sm">
				{/* The switcher scrolls inside the header instead of widening the pane — see SPEC.md. */}
				<div className="flex min-w-0 flex-1 items-center gap-sm overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
					{SURFACES.map((option) => (
						<ToggleSegment
							key={option.value}
				...
				<IconTooltip label="Re-read configuration">
					<button
						...
						className="flex size-5 shrink-0 items-center justify-center rounded-[var(--radius-sm)] text-text-muted hover:bg-control-bg-hovered hover:text-text-default"
					>
			...
			<QuietScrollArea className="min-h-0 flex-1">
				{!snapshot ? (
					...
					<ClaudeAccountSurface revision={tick + reloads} reloads={reloads} />
				) : (
