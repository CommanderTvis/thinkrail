...
import { IconTooltip, ToggleSegment } from "@thinkrail/plugin-ui";
...
import { createClaudeAccountSurface } from "./ClaudeAccountSurface";
...
import { createClaudeEditDialog } from "./ClaudeEditDialog";
import { createClaudeMarketplaceDialog } from "./ClaudeMarketplaceDialog";
import { createClaudePluginMoveDialog, type PluginMove } from "./ClaudePluginMoveDialog";
import {
	createClaudePluginUninstallDialog,
	type PluginUninstall,
} from "./ClaudePluginUninstallDialog";
...
// An `@`-import nests under the file that pulled it in — see SPEC.md for how it is drawn.
...
function ContextSurface({
	...
	// invited a precision the pane never had — see SPEC.md.
	...
	return (
		<div className="flex flex-col">
			...
			{layers.map((layer, index) => {
				...
				const last = (layers[index + 1]?.depth ?? 0) < depth;
				const hasChild = (layers[index + 1]?.depth ?? 0) > depth;
				return (
					<div
						key={layer.path}
					...
					>
						{indentGuides(layer).map((guide, level, all) =>
							level === all.length - 1 ? (
								<span
									key={guide}
									aria-hidden="true"
									data-connector={last ? "last" : "tee"}
									className={`relative ml-15 w-5 shrink-0 after:absolute after:top-1/2 after:-right-8 after:left-0 after:h-px after:bg-border-default after:content-[''] ${
										last
											? "before:absolute before:top-0 before:left-0 before:h-1/2 before:w-px before:bg-border-default before:content-['']"
								...
								<span
									key={guide}
									aria-hidden="true"
									className="ml-15 w-5 shrink-0 border-border-default border-l"
								/>
							),
						)}
						<div className="flex min-w-0 flex-1 items-center gap-8 py-4 pr-8 pl-8">
							<span
								className={`relative -my-4 flex shrink-0 items-center self-stretch ${
									hasChild
										? "before:absolute before:top-[calc(50%+7px)] before:bottom-0 before:left-7 before:w-px before:bg-border-default before:content-['']"
										: ""
								}`}
							>
								<FileText className="size-14 text-text-subtle" />
							</span>
...
export function createClaudeConfigPanel(ctx: Ctx) {
	const ClaudeAccountSurface = createClaudeAccountSurface(ctx);
	const ClaudeEditDialog = createClaudeEditDialog(ctx);
	const ClaudeMarketplaceDialog = createClaudeMarketplaceDialog(ctx);
	const ClaudePluginMoveDialog = createClaudePluginMoveDialog(ctx);
	const ClaudePluginUninstallDialog = createClaudePluginUninstallDialog(ctx);

	return function ClaudeConfigPanel({ workspaceId }: { workspaceId: string }) {
		const [pendingEdit, setPendingEdit] = useState<Parameters<EditRequest>[0] | null>(null);
		...
		// What Claude itself reaches beyond the files: claude.ai connectors and plugin servers, asked of its
		// CLI after the snapshot, since that answer costs a health check and must not hold the pane.
		const [reached, setReached] = useState<ClaudeCapability[]>([]);
		const [error, setError] = useState<string | null>(null);
		const [surface, setSurface] = useState<Surface>("context");
		// Every reload only ever climbs, so it is a revision the account surface can watch.
		const [reloads, setReloads] = useState(0);
		const openSource = useOpenSource(ctx, workspaceId);
			...
			(generation: number) => {
				...
				void ctx
					...
					.then((result) => {
						...
						setError(null);
						void ctx
							.request("mcpList", { workspaceId })
							.then((listed) => {
								if (current && generation === applied.current) setReached(listed.capabilities);
							})
							.catch(() => {});
					})
					.catch((cause: unknown) => {
		...
		return (
			...
			>
				...
				<ClaudeEditDialog
					...
					key={pendingEdit ? JSON.stringify(pendingEdit) : "idle"}
					workspaceId={workspaceId}
				...
				<div className="min-h-0 flex-1 overflow-auto">
					{!snapshot ? (
						...
						<ClaudeAccountSurface reloads={reloads} />
					) : (
						<ClaudeCapabilitiesSurface
							capabilities={[...snapshot.capabilities, ...reached]}
