...
describe("isToolShowing", () => {
	test("a tool shows only from a visible side, an unfolded group, and that group's shown tab", () => {
		const document = baseDocument();
		const attention = reconcileAttention(document, undefined);
		expect(isToolShowing(document, attention, "projects")).toBe(true);
		expect(isToolShowing(document, attention, "changes")).toBe(false);

		const hidden = { ...document, left: { ...document.left, visible: false } };
		expect(isToolShowing(hidden, attention, "projects")).toBe(false);

		const folded = setSideGroupFolded(document, "left", "left-a", true);
		if (isLayoutUnavailable(folded)) throw new Error(folded.reason);
		expect(isToolShowing(folded.document, attention, "projects")).toBe(false);
				...
				groups: [
					{
						...
						weight: 1,
						folded: false,
						tabs: [toolTab("projects"), toolTab("changes")],
					},
				],
			},
		};
		const seeded = reconcileAttention(shared, undefined);
		expect(isToolShowing(shared, seeded, "projects")).toBe(true);
		const behind = selectTab(seeded, { area: "left", groupId: "left-a" }, toolTab("changes").id);
		expect(isToolShowing(shared, behind, "projects")).toBe(false);
		expect(isToolShowing(shared, behind, "changes")).toBe(true);
	});
});
