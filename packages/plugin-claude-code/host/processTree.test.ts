...
describe("runsInsideAgent", () => {
	const rows = (inner: string, outer: string): ProcessRow[] => [
		{ pid: 10, ppid: 1, name: "zsh" },
		{ pid: 20, ppid: 10, name: outer },
		{ pid: 30, ppid: 20, name: "sh" },
		{ pid: 40, ppid: 30, name: inner },
	];

	test("an agent another agent started belongs to that agent's tab", () => {
		expect(runsInsideAgent(snapshotFromRows(rows("claude", "codex")), 10, "claude", "codex")).toBe(
			true,
		);
	});

	test("the outer agent, or one with no agent above it, is the tab's own", () => {
		expect(runsInsideAgent(snapshotFromRows(rows("codex", "claude")), 10, "claude", "codex")).toBe(
			false,
		);
		expect(runsInsideAgent(snapshotFromRows(rows("claude", "node")), 10, "claude", "codex")).toBe(
			false,
		);
	});
});
