...
import type { AskUserAnswersMessage, AskUserQuestionAckDetails } from "./types";
import { ASK_USER_ANSWERS_CUSTOM_TYPE, isAskUserAnswersMessage } from "./types";
...
let questionnaireAsk: QuestionnaireAsk | undefined;

export function setQuestionnaireAsk(ask?: QuestionnaireAsk): void {
	questionnaireAsk = ask;
}
	...
	return {
		...
		async execute(toolCallId, params, signal, _onUpdate, ctx: ExtensionContext) {
			...
			if (questionnaireAsk) {
				...
				const answers = await questionnaireAsk(sessionId, toolCallId, args, signal);
				return buildQuestionnaireResponse(answers, args);
			}
			return {
