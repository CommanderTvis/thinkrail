	...
	return function CodexSettings() {
		...
		return (
			<section data-testid="settings-codex" className="flex flex-col gap-16">
				...
				<button
					type="button"
					data-testid="codex-ide-context-toggle"
					aria-pressed={ideContext}
					onClick={() => update({ ideContext: !ideContext })}
				...
				>
					<span className="flex flex-col gap-2">
						<span className="tr-title-compact text-text-default">Start with IDE context</span>
						<span className="text-text-muted tr-text-metadata">
							Runs /ide on when the Codex Launcher starts a session.
						</span>
					</span>
					{ideContext ? <Check className="size-16 shrink-0 text-primary" /> : null}
				</button>
