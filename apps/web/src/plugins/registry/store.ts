...
interface PluginRegistryState {
	...
	setRoster: (roster: PluginRosterEntry[]) => void;
...
function buildToolCatalog(
	...
	return roster.flatMap((entry) =>
		entry.contributes.sideTools.map((tool) => ({
			id: tool.tool,
			label: tool.label,
			icon: pluginIcon(tool.icon),
			activeIcon: pluginIcon(tool.icon, true),
...
export const usePluginRegistry = create<PluginRegistryState>((set) => ({
	...
	setRoster: (roster) => set((s) => ({ roster, toolCatalog: buildToolCatalog(roster, s.active) })),
	setActive: (pluginId, active) =>
		set((s) => {
			const next = new Set(s.active);
			if (active) next.add(pluginId);
			else next.delete(pluginId);
			return { active: next, toolCatalog: buildToolCatalog(s.roster, next) };
		}),
	...
	removePlugin: (id) =>
		set((s) => {
			...
			return {
				...
				toolCatalog: buildToolCatalog(s.roster, active),
