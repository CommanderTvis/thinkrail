...
export const PDF_MAX_SCALE = 6;
export const PDF_SCALE_STEP = 1.15;
 ...
 */
export function pdfScaleForWheel(scale: number, deltaY: number, deltaMode = 0): number {
	const pixels = deltaY * (DELTA_MODE_PIXELS[deltaMode] ?? 1);
	const bounded = Math.max(-PDF_ZOOM_MAX_DELTA, Math.min(PDF_ZOOM_MAX_DELTA, pixels));
	return clampPdfScale(scale * Math.exp(-bounded / PDF_ZOOM_SENSITIVITY));
}
 ...
 */
export function isPdfZoomGesture(event: { ctrlKey: boolean; metaKey: boolean }): boolean {
