...
export function ClaudeCodeSettings() {
	...
	const disableAgentView = useAppStore((s) => s.claudeDisableAgentView);
	const setDisableAgentView = (on: boolean) => {
		getTransport()
			.request("settings.update", { config: { claudeDisableAgentView: on } })
			.catch(() => toast.error("Couldn't change the agent-view setting"));
	};
