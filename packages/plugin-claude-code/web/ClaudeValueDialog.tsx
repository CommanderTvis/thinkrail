...
import type { ClaudeEdit, ClaudeSettingInput } from "../contracts";
...
function ListEntries({ items, onChange }: { items: string[]; onChange: (next: string[]) => void }) {
	...
	// The row's identity, not its position, is what a key must track — ids stay put across an edit and
	// only change when a row is actually added or removed, both handled locally below.
