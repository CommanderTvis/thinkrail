...
test("external file access follows the active plugin and current workspace declaration", async () => {
	const root = tempRoot();
	writeFixturePlugin(root, "fixture");
	const path = join(root, "config.toml");
	const hostPath = join(root, "fixture", "host.js");
	writeFileSync(
		hostPath,
		readFileSync(hostPath, "utf8").replace(
			'ctx.method("ping", () => ({ pong: true }));',
			'ctx.externalFiles((workspaceId) => workspaceId === "ws" ? [' +
				JSON.stringify(path) +
				"] : []);",
		),
	);
	...
	const runtime = await installPlugins(seams);
	try {
		expect(runtime.allowsExternalFile("ws", path)).toBe(true);
		expect(runtime.allowsExternalFile("other", path)).toBe(false);
		expect(runtime.allowsExternalFile("ws", join(root, "auth.json"))).toBe(false);
		seams.config().plugins.fixture = { enabled: false };
		await runtime.reconcile();
		expect(runtime.allowsExternalFile("ws", path)).toBe(false);
		seams.config().plugins.fixture = { enabled: true };
		await runtime.reconcile();
		expect(runtime.allowsExternalFile("ws", path)).toBe(true);
	} finally {
		runtime.dispose();
