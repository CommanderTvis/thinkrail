...
Activation-scoped `externalFiles` providers supply exact absolute paths for a workspace.
`PluginRuntime.allowsExternalFile` consults active registrations only; disablement removes access.
The host routes permitted paths through its shared text-file I/O without depending on any plugin id.
