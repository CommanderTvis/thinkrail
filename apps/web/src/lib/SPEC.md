...
- **Owns:** `utils.ts` → `cn()` (merge clsx output through tailwind-merge) + `isMarkdownPath()` (the
  ...
  **`LayoutAttention`** device-local overlay shared by store, shell, and the headless layout child, with
  ...
  mid-session switch can never offer different models. `withLaunchEnv()` prefixes a launch with
  `CLAUDE_CODE_DISABLE_AGENT_VIEW=true` **in the syntax of the shell that will read it** — a POSIX
  prefix typed into `cmd` is a command not found, and the Windows shell is already a setting, so the
  three forms are pinned by unit test rather than assumed. **`claudeModelPicker.ts`** is that mid-session
  ...
  puts up its own "Change effort level?" over a cached conversation, and the effort drive left it there —
  the chip reported the switch that never happened. One prompt matcher covers both, and both drives end by
  answering it.
...
- **Public surface (barrel):** `cn`, `isMarkdownPath`, `isPdfPath`, `stripFrontmatter`, `cssColorToHex`,
  ...
  `claudeLaunchCommand`, `shellQuotePath`, `withLaunchEnv`, `CLAUDE_AGENT_VIEW_ENV`, `CLAUDE_LAUNCH_MENU`, `composerDraft`, `FILE_DRAG_TYPE`, `startFileDrag`,
