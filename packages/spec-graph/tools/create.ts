...
import { defineSpecTool, errorResult, getIndex, scaffoldBody, textResult } from "./shared.ts";
...
export const specCreate = defineSpecTool<
	typeof parameters,
	{ path: string; id: string } | { error: string }
>({
	name: "spec_create",
	label: "Spec Create",
	description:
	...
	async run(params, cwd) {
		const resolved = resolveSpecPath(cwd, params.path);
