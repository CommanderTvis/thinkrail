...
### Claude Code integration, builtin ✅
...
happens to index it. It is the worked example of one builtin plugin depending on another. Needs S1,
S2, S3, S4, S5, S6, S10, S11, and the spec dialect first.

### Discord Rich Presence, builtin ✅

The IPC client, the pure presence decision, and the connection lifecycle move unchanged (into
`host/lifecycle.ts`, closure-scoped per activation rather than a module singleton); two methods and a
state channel (`plugin.discord.presence`/`plugin.discord.status`, snapshot channel `status`); the
snowflake validation the fork inlined into core config loading becomes its settings schema; the
whole-store subscription becomes a `ctx.watchHost` selector. `DiscordSettings.enabled` and
`DiscordConnectionState`'s `"off"` member drop out entirely — a plugin's settings schema may not declare
`enabled` (`validateContractIntake`), and the generic Settings › Plugins toggle already gates activation,
so there is no second on/off switch to keep in sync. The smallest port, chosen to prove builtin loading
end to end. Needed S5, S6, S10, S11.

### PDF preview, builtin ✅

Web only, with no host half — `@thinkrail/plugin-pdf-preview`. Its viewer is declared in
`contributes.fileViewers` so the dispatcher knows the read strategy (`read: "none"`) before the module
loads, and the component comes from the web half. W14's `ctx.fileUrl` replaces the private URL builder;
`FileViewerProps.revision` (already the file's own byte tick) replaces the hand-rolled live reload, so
neither `ctx.useFileRevision` nor `ctx.watchWorkspace` was needed. The zoom gesture math came from the
kit's existing `@thinkrail/plugin-ui/visualization` subpath rather than the app; there was no find bar to
move. Landing this as a *builtin* rather than external (S12) surfaced a gap this document did not
anticipate: the loader could not yet represent a builtin with a manifest but no host module at all.
`packages/server/src/plugins/registry.ts`'s `registerBuiltinManifest` and the matching relaxation in
`activation.ts`'s `activate()` are the smallest fix — see `packages/server/src/plugins/SPEC.md`.

### Visualize beside a terminal, builtin ✅

A builtin plugin wrapping an upstream pi extension rather than an upstream plugin: the tool is registered
...
the result; and drawings are persisted per agent session (over `ctx.readState`/`writeState`), adopted when
a resumed agent reports its id (`ctx.onTerminal`'s `agentChanged`). On the web: a companion rendering the
kit's visualization card, with its per-drawing title carried through the new `CompanionRegistration.useTitle`
hook rather than a core special case. `@thinkrail/plugin-visualize`, `plugin-visualize/SPEC.md`.

### Branch graph, landed

"A graph of the project's branches, in the rail" renders a commit graph as a side tool, with its own
lane-assignment algorithm, over a `plugin.branch-graph.graph` read keyed by project rather than workspace
(moved from core's `git.graph`).
...
reason H12 covers projects and not only workspaces, because the read is project-scoped. `@thinkrail/plugin-branch-graph`, `plugin-branch-graph/SPEC.md`.
...
the `todo.*` wire methods and the plan document tab. `pi-visualize` stays in core as a package, wrapped by
the builtin `plugin-visualize`. Web-access is a third-party npm package the host adapts with a policy
hook, so it is not ours to package.

## Landed

S1 through S13 and the eight plugins above — the spec dialect, Claude Code integration, Blueprint, PDF
preview, file-type icons, Discord Rich Presence, Visualize beside a terminal, and the branch graph, all
builtin — have landed: `@thinkrail/plugin-api`, `@thinkrail/plugin-ui`, the server and web loaders, the
Settings › Plugins section, and the external-plugin path exercised end to end by
`e2e/plugins/fixture/external-plugin.spec.ts` against a checked-in fixture plugin.

A few things landed differently from how this document first described them.

- **File-type icons landed as `packages/plugin-file-icons`**, the provisional plugin this document
  describes, occupying W17's `fileIcon` core slot: material-icon-theme's set, the filename/extension
  lookup, and the recolouring generator all moved out of `apps/web`; `components/FileTypeIcon.tsx` is now
  just the slot lookup plus a Remix fallback. S13 landed as both the slot and its first occupant.
- **W10 is two named registration shapes, not one with an optional field.** The web contract exports
  `WorkspaceScopedActionRegistration` (`scope?: "workspace"`) and `ProjectScopedActionRegistration`
  (`scope: "project"`), unioned as `WorkspaceActionRegistration`, so a project-scoped action's extra
  requirement (it also renders on the Welcome screen and after project creation) is a type-level
  distinction rather than a runtime branch on an optional field.
- **W17 is "register a resolver into a named core slot," not a single file-icon capability.** The web
  contract's `CoreSlots` carries `fileIcon`, `documentLink`, and `writtenPathGroup`; the spec dialect
  registers the latter two, and `fileIcon` has no registrant (see above). The capability generalized
  during implementation rather than shipping as the narrower, file-icon-only shape this document and the
  module spec's earlier drafts described.
- **An external plugin bundles its own dependencies rather than resolving them from the host.** A host
  module imported from a directory outside the repo cannot resolve bare specifiers, so `host.js` is
  self-contained (hand-written schemas or a bundled build, `packages/server/src/plugins/SPEC.md`), and a
  web half reads React, ReactDOM, and the kit off `window.__thinkrailPluginRuntime` instead of an import
  map — the registry needs no browser floor this project has not established, where an import map would.
  Both are the mechanism `e2e/fixtures/plugin-fixture/` demonstrates, not a deviation this document
  anticipated in that much mechanical detail.
