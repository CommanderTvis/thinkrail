						...
						{indentGuides(layer).map((guide, level, all) =>
							level === all.length - 1 ? (
								<span
									...
									className={`relative ml-8 w-12 shrink-0 after:absolute after:top-1/2 after:-right-8 after:left-0 after:h-px after:bg-border-default after:content-[''] ${
										last
	...
	return (
		...
		>
			<div className="flex h-panel-header-row shrink-0 items-center gap-8 px-8">
				...
				<div className="flex min-w-0 flex-1 items-center gap-8 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
					{SURFACES.map((option) => (
						<ToggleSegment
							key={option.value}
