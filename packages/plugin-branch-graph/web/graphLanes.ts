import type { GitGraphCommit } from "../contracts";
...
export interface GraphRow {
	...
	toBelow: boolean;
	/** Lanes this commit opens below itself, one per parent after the first — see SPEC.md. */
	branches: number[];
...
/** Lanes for a commit list already in date order — see SPEC.md for the rule. */
export function layoutGraph(
	...
	for (const commit of commits) {
		...
		for (const parent of rest) {
			const opened = claim(parent);
			if (opened !== lane && !branches.includes(opened)) branches.push(opened);
		}
		...
		rows.push({
			...
			branches,
			width: branches.reduce((widest, branch) => Math.max(widest, branch), Math.max(width, lane)),
		});
	}
