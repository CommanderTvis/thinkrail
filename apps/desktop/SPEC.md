...
layout — it lives in `@thinkrail/shared/claudePlugin`, shared by `preBuild.ts` that writes it, the app that
points at it, and the artifact smoke that proves it shipped; its test pins the parent relationship the host
relies on.
