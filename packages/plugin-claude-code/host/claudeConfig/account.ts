...
import { runBounded } from "@thinkrail/shared/runBounded";
import type { ClaudeAccount, ClaudeUsageSeverity, ClaudeUsageWindow } from "../../contracts";
 ...
 * this pane reads. Run only when a person asks for it — see SPEC.md.
 */
...
export async function readClaudeAccount(
	...
	const versionRun = await runBounded([claudeBinary(claudeCommand), "--version"], {
		timeoutMs: TIMEOUT_MS,
		env: process.env,
	});
	const version = versionRun.ok ? text(versionRun.out.trim().split(/\s+/)[0]) : undefined;
