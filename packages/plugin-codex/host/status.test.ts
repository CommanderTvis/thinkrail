...
test("a hook payload maps to a status and keeps the facts it carries", () => {
	...
	expect(
		...
		status: "done",
		lastMessage: "ok",
		transcriptPath: "/c/rollout.jsonl",
	});
});
...
test("a menu preset puts its subcommand first and its flags after the MCP override", () => {
	...
	expect(find("yolo")).toBeUndefined();
});

test("saved permissions apply to ordinary launches and yield to permission presets", () => {
	const presets = codexLaunchMenu(codexEnumValues).flat();
	const options = { mcp: false, windows: false, permissionMode: "sandbox-danger-full-access" };
	expect(codexLaunchLine("codex", options)).toBe("codex -s danger-full-access");
	expect(codexLaunchLine("codex", { ...options, resume: {} })).toBe(
		"codex resume --last -s danger-full-access",
	);
	for (const id of ["search", "model-gpt-6-astra", "fork"]) {
		const preset = presets.find((item) => item.id === id);
		expect(codexLaunchLine("codex", { ...options, preset })).toContain("-s danger-full-access");
	}
