...
assert_json_field "no transcript, no transcript_path" "$PAYLOAD" ".transcript_path" "null"

PAYLOAD=$(build_payload '{"session_id":"s1","transcript_path":"/custom/config/projects/-p/s1.jsonl"}' "stop")
assert_json_field "transcript_path passed through as Claude reported it" "$PAYLOAD" ".transcript_path" "/custom/config/projects/-p/s1.jsonl"
