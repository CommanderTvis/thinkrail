...
- **Owns:** `store.ts` (`usePluginRegistry`: `manifests`, `roster`, `active`, one append-only, pluginId-tagged
  ...
  active?)` — a curated Remix Icon lookup by manifest icon name, `active` picking the `Fill` variant where
  the map has one, `"claude"` resolved through the `fileIcon` slot (path `"CLAUDE.md"`) and falling back to
  `RiRobot2Line` when no plugin answers, everything else unknown falling back to `RiPuzzle2Line`).
- **Public surface (barrel):** `usePluginRegistry`, every selector above, `pluginIcon`, and the
