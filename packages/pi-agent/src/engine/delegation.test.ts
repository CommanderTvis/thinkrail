...
test("host embedding: projection, per-workspace service, transcript store, cascades, retention", async () => {
	...
	const service = delegationServiceFor(cwd);
	expect(delegationServiceFor(cwd)).toBe(service);
	expect(delegationServiceFor(tmpDir("trdel-other-"))).not.toBe(service);
	...
	expect(child.record.sessionFile.startsWith(join(delegationRootDir(), delegationScope(cwd)))).toBe(
		true,
	);

	const read = readChildTranscript(cwd, sessionId, child.sessionId);
	...
	const compacted = readChildTranscript(cwd, sessionId, child.sessionId);
	expect(compacted.messages.some((message) => message.role === "assistant")).toBe(true);

	await removeSession(sessionId);
	expect(service.findChild(child.sessionId)).toBeUndefined();
	const afterDispose = readChildTranscript(cwd, sessionId, child.sessionId);
	expect(afterDispose.messages.length).toBeGreaterThan(0);
	expect(afterDispose.status).toBeUndefined();

	await removeCwdSessions(cwd);
	let missing: unknown;
	try {
		readChildTranscript(cwd, sessionId, child.sessionId);
	} catch (err) {
		missing = err;
	}
...
test("deleteSession resolves only after its child cascade settles", async () => {
	...
	let deleted = false;
	const done = deleteSession(sessionId, cwd).then(() => {
		deleted = true;
	});
...
test("workspace archival awaits in-flight delete transactions before deleting the store", async () => {
	...
	const done = deleteSession(sessionId, cwd);
	await waitFor(() => liveParentContext(sessionId) === undefined);
	const archived = removeCwdSessions(cwd);
	release();
...
test("children follow their parent's retained runtime generation across a flip", async () => {
	...
	const { sessionId: oldParent } = await createSession({ cwd: cwdOld });
	const service = delegationServiceFor(cwdOld);
	...
	try {
		...
		({ sessionId: newParent } = await createSession({ cwd: cwdNew }));
...
test("transcript reads reject path-like ids — wire strings never escape the delegation root", () => {
	expect(() => readChildTranscript("/tmp/ws", "..", "c")).toThrow("Invalid parentSessionId");
	expect(() => readChildTranscript("/tmp/ws", "p", "x/../y")).toThrow("Invalid childSessionId");
	expect(() => readChildTranscript("/tmp/ws", "a\\b", "c")).toThrow("Invalid parentSessionId");
	expect(() => readChildTranscript("/tmp/ws", "p", "c/d")).toThrow("Invalid childSessionId");
});
