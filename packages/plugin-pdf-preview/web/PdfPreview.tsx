...
import {
	...
	ZOOM_SCALE_STEP,
	zoomScaleForWheel,
} from "@thinkrail/plugin-ui/visualization";
 ...
 * text a reader wants to select is a second layer over it. See SPEC.md.
 */
function PdfPage({
	doc,
	pageNumber,
	...
	// produced are stretched, and the sharp ones arrive when the gesture settles. See SPEC.md.
	useEffect(() => {
...
export default function PdfPreview({ url, cacheBust }: { url: string; cacheBust: number }) {
	...
	// way a file changes — a reload asks for the bytes again without closing the tab. See SPEC.md.
	...
	useEffect(() => {
		...
		const task = loadPdf(`${url}?t=${cacheBust}.${reloads}`);
		task.promise
			.then((next) => {
	...
	}, [url, cacheBust, reloads]);

	// The gesture is continuous; rasterizing is not. Redrawing on every wheel event is what made a pinch
	// stutter, so the pages are re-rendered once the scale stops moving. See SPEC.md.
	useEffect(() => {
