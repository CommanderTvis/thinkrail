import { RiFileCodeLine, RiGitCommitLine } from "@remixicon/react";
...
export function createGraphPanel(ctx: Ctx) {
	return function GraphPanel({ workspaceId }: { workspaceId: string }) {
		...
		const copyHash = async (sha: string) => {
			...
			if (!ok) ctx.notify("error", "Could not copy commit hash");
		};

		const copyPatch = async (sha: string) => {
			if (!projectId) return;
			try {
				const { patch } = await ctx.request("patch", { projectId, sha });
				const ok = await copyToClipboard(patch);
				if (!ok) ctx.notify("error", "Could not copy patch to clipboard");
			} catch (err) {
				ctx.notify(
					"error",
					"Could not copy patch",
				...
				>
					<div className="absolute inset-x-0" style={{ top: first * ROW_HEIGHT }}>
						{rows.slice(first, last).map((row) => {
							...
							return (
								<ContextMenu key={row.commit.sha}>
									...
									<ContextMenuContent data-testid="graph-commit-menu">
										<ContextMenuItem
											data-testid="graph-copy-hash"
											onSelect={() => void copyHash(row.commit.sha)}
										>
											<RiGitCommitLine />
											Copy commit hash
										</ContextMenuItem>
										<ContextMenuItem
											data-testid="graph-copy-patch"
											onSelect={() => void copyPatch(row.commit.sha)}
										>
											<RiFileCodeLine />
