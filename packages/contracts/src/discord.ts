export interface DiscordSettings {
	 ...
	 * mode — Discord shows the *application's* name — so this ships as ThinkRail's own registered
	 * application and a user who wants their own name on it pastes that id instead. Empty means "not
	 * configured", which is a choice the user has to make rather than the state they start in.
	 */
...
/** ThinkRail's own registered Discord application, so the presence has a name to wear out of the box. */
export const THINKRAIL_DISCORD_APPLICATION_ID = "1542175111384404138";

export const DEFAULT_DISCORD_SETTINGS: DiscordSettings = {
	enabled: false,
	applicationId: THINKRAIL_DISCORD_APPLICATION_ID,
	blockedProjectIds: [],
	shareFileName: true,
};
