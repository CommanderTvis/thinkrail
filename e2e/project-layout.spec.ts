...
test("live terminals survive returning after another project's layout changes", async ({
	page,
}) => {
	await openFixtureProject(page);
	await enterDefaultWorkspace(page);
	...
	await runInTerminal(page, "echo TR_PROJECT_TERMINAL_SURVIVES");
	await expect(visibleTerminalScreen(page)).toContainText("TR_PROJECT_TERMINAL_SURVIVES");
