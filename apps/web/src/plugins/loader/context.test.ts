...
beforeEach(() => {
	...
	useAppStore.setState({
		...
		layoutAttentionByWorkspace: {
			w1: {
				selectedByGroup: {},
				lastFocusedCenterGroupId: "center-main",
				lastFocusedSideGroupId: {},
				navigationClockByGroup: { "center-main": 0 },
			},
		},
...
test("openTerminal places and selects a fresh tabKey", async () => {
	...
	expect(useAppStore.getState().layoutIntents).toContainEqual(
		expect.objectContaining({
			kind: "place-terminal",
			tabKey: "t1",
			targetGroupId: "center-main",
		}),
	);
});

test("openTerminal places a tabKey that exists in the host catalog but not the layout", async () => {
