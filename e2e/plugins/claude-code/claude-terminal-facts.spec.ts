...
import { E2E_HOME_DIR } from "../../fixtures/paths";
...
test("the attach chip types a Claude @path for a file the host's own picker returns", async ({
	page,
}) => {
	await openFixtureProject(page);
	const workspace = await createWorkspaceViaDialog(page);
	...
	await page.getByTestId("claude-plugin-dismiss").click();
	// There is no in-app worktree browser to pick from — every attach goes through the host's own picker,
	// stubbed by the harness to answer a fixed path outside the worktree. See plugins/claude-code/SPEC.md.
...
test("a Claude terminal shows what its session has spent, read from its own transcript", async ({
	page,
}) => {
	await openFixtureProject(page);
	const workspace = await createWorkspaceViaDialog(page);
	...
	const sessionId = "0e5f4a52-7d1c-4c2e-9d0b-1f6a7c3e9b21";
	...
	writeFileSync(transcript, `${turn("msg_a", 5)}${turn("msg_a", 1_200)}`);

	const chip = page.locator('[data-testid="terminal-agent-fact"][data-kind="usage"]');
	await runInTerminal(
		page,
		report({ event: "prompt_submit", session_id: sessionId, cwd: workspace.worktreePath }),
	);
	await expect(chip).toHaveText("↑2 · ↓1.2k · R12k · W500");

	appendFileSync(transcript, turn("msg_b", 800));
	await runInTerminal(
		page,
		report({ event: "stop", session_id: sessionId, cwd: workspace.worktreePath }),
	);
	await expect(chip).toHaveText("↑4 · ↓2.0k · R24k · W1.0k");
});
