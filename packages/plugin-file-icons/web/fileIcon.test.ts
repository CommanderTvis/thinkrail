...
import { createFileIconResolver } from "./index";
...
test("the slot resolves the same component for the same icon, so re-renders never remount it", () => {
	const resolve = createFileIconResolver((path) => `/assets/${path}`);
	const first = resolve("src/a.ts", "file");
	expect(first).not.toBeNull();
	expect(resolve("lib/b.ts", "file")).toBe(first);
	expect(resolve("readme.md", "file")).not.toBe(first);
	expect(resolve("src", "directory")).toBeNull();
});
