...
export interface IdeBridgeDeps {
	...
	/** The workspace a connecting agent's process belongs to, or null when we did not spawn it. */
...
/** A client is only told about a file once it has said which workspace it is in — see SPEC.md. */
const clientWorkspaces = new WeakMap<Socket, string>();
...
function broadcast(workspaceId: string, message: string): void {
	for (const socket of clients) {
		if (clientWorkspaces.get(socket) === workspaceId) socket.send(message);
	}
}
...
async function handleMessage(socket: Socket, raw: string): Promise<void> {
	...
	// The one notification worth reading: it names the agent's own process, which is how a socket learns
	// which workspace it belongs to. A process we did not spawn stays untagged and hears nothing.
	if (method === "ide_connected") {
		const params = (message.params ?? {}) as { pid?: unknown };
		const pid = typeof params.pid === "number" ? params.pid : 0;
		const workspaceId = deps?.workspaceForProcess(pid) ?? null;
		if (workspaceId !== null) clientWorkspaces.set(socket, workspaceId);
		return;
	}
	...
	const server = Bun.serve<{ authorized: boolean }, never>({
		...
		websocket: {
			...
			close(ws) {
				...
				clientWorkspaces.delete(ws as unknown as Socket);
			},
		},
	});
