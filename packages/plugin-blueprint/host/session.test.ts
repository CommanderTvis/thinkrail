...
setBlueprintPublisher((payload) => {
	if (payload.state) frames.push(payload.state);
});
...
test("an existing BLUEPRINT.md on disk with no prior session record is restored", () => {
	writeDoc(DOC);
	resetBlueprintsForTest();

	const restored = getBlueprint("fresh-workspace", worktree);
