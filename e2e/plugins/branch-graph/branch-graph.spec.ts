...
test("commit right-click menu copies commit hash and patch to clipboard", async ({
	page,
	context,
}) => {
	...
	const commit = page.getByTestId("graph-commit").filter({ hasText: "work on feature-one" });
	await expect(commit).toBeVisible();
	const shortSha = await commit.getAttribute("data-sha");
	const targetCommit = page.locator(`[data-testid="graph-commit"][data-sha="${shortSha}"]`);

	await targetCommit.click({ button: "right" });
	const menu = page.getByTestId("graph-commit-menu");
	await expect(menu).toBeVisible();

	await page.getByTestId("graph-copy-hash").click();
	await expect(menu).toBeHidden();
	const copiedHash = await page.evaluate(() => navigator.clipboard.readText());
	expect(copiedHash).toMatch(/^[0-9a-f]{40}$/);
	expect(copiedHash.startsWith(shortSha ?? "")).toBe(true);

	await targetCommit.click({ button: "right" });
	await expect(menu).toBeVisible();

	await page.getByTestId("graph-copy-patch").click();
	await expect(menu).toBeHidden();
	await expect
		.poll(async () => page.evaluate(() => navigator.clipboard.readText()))
		.toContain("From ");
	const copiedPatch = await page.evaluate(() => navigator.clipboard.readText());
	expect(copiedPatch).toContain(`From ${copiedHash}`);
	expect(copiedPatch).toContain("[PATCH]");
});
