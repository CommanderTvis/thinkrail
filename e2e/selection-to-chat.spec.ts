...
test("a highlight in the rendered markdown preview attaches to the composer too", async ({
	page,
}) => {
	await openFixtureProject(page);
	await createWorkspaceViaDialog(page);
	...
	await page
		...
		.getByRole("heading", { name: "sample-project" })
		.evaluate((heading) => document.getSelection()?.selectAllChildren(heading));
	const chip = page.getByTestId("composer-selection");
	await expect(chip).toBeVisible();
	await expect(chip).toContainText("README.md:1");

	// Reaching for the composer collapses the document selection; the chip must survive that.
	await page.getByTestId("chat-input").fill("what is this?");
	await expect(chip).toBeVisible();
	await page.getByTestId("chat-input").press("Enter");
	const sent = page.locator('[data-testid="chat-message"][data-role="user"]').last();
	await expect(sent).toContainText("README.md:1");
	await expect(sent).toContainText("sample-project");
	await expect(chip).toHaveCount(0);
});
