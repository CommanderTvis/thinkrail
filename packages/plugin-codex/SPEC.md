...
none (the agent record's session id is set once). The Codex mark is its own glyph, not ChatGPT's OpenAI
knot: `assets/codex.svg` is lobehub's `@lobehub/icons-static-svg` `codex` icon (MIT), stripped to
`currentColor`. The manifest names it `asset:codex.svg` for core's rail and plugin list, and
`web/CodexGlyph.tsx` (`createCodexGlyph(ctx)`) draws the same file through the kit's `SvgAsset`,
falling back to Remix's OpenAI mark when the asset cannot be fetched.

## Token totals and the plan come from the rollout (`host/rollout.ts`)

Every hook's stdin names the session's rollout as `transcript_path`, so each report makes the host read
that file forward from where it last stopped (a byte offset per path, complete lines only), keeping two
facts Codex itself wrote: the last `event_msg` `token_count`'s `info.total_token_usage` (the session's
spending: input minus `cached_input_tokens`, which is shown as cache read, plus output and
`cache_write_input_tokens`), and the last `response_item` `function_call` named `update_plan`, whose
`arguments` JSON `{plan:[{step,status}]}` becomes the terminal's plan. Both ride the status push as
`usage`/`plan`; the web store keeps the last value when a push carries neither, and the accessory shows
them through the kit's `TerminalUsageChip`/`TerminalPlan`. Nothing is estimated and no cost is shown. The
`token_count` shape was verified against a codex-cli 0.155.1 rollout; the `update_plan` call shape
follows Codex's documented tool arguments and the verified `function_call` envelope, but no live rollout
containing one has been checked yet. A malformed plan or unknown status is dropped rather than guessed.
The offset state is per rollout path and dropped when the last terminal reporting it closes.
...
The launcher and its terminal model picker read `model/list` from a retained Codex app-server connection,
not a source-controlled alias list. The response's `model` is the launch argument and `displayName` is the
label; hidden entries stay out. A catalog read failure leaves no model choices rather than offering stale
or invented IDs. The generic Start work launcher deliberately keeps Codex on its configured default until
the plugin API supports a reactive model catalog.
...
Settings → Codex also has **Start with IDE context**, off by default. It applies only to Codex Launcher
sessions, never to a manually opened Codex terminal. Once that terminal exists, ThinkRail writes Codex's
own `/ide on` command to it rather than pretending a CLI flag or terminal environment variable controls
the setting: Codex has neither, as tracked by [openai/codex#30181](https://github.com/openai/codex/issues/30181).
Each detected Codex terminal also has an IDE context chip which issues `/ide on` or `/ide off` for that
session only; its choice is never saved.
...
(`--sandbox workspace-write --ask-for-approval on-request`), and live web search. There is no
`--yolo` preset: it bypasses approvals *and* the sandbox, and `-s danger-full-access` already covers
the need for full access. A preset's subcommand goes first on the line and its flags after
the MCP override.
...
Codex's right-click launcher menu and running-session model picker use the local CLI's `model/list`
catalog. Selecting a model adds its reported `model` value as `--model` for that launch only; left click
omits an override, preserving the configured default. Start work currently omits a Codex model picker
instead of presenting a stale hard-coded list.

The terminal's model chip switches a running session by driving Codex's `/model` picker, and only for
that session: it takes a row with `s` (or, for a model with a choice of efforts, opens the effort list
and takes the highlighted effort with `s`). It never presses Enter to accept, because Enter saves the
choice as the default in `config.toml`. The `s` key arrived in Codex 0.156 (openai/codex#45831); on an
older Codex the row is found but `s` does nothing, so the switch backs out and says Codex must be
updated, rather than claiming the model is not offered or overwriting the user's default.
...
- **Owns:** `manifest.ts`, `contracts.ts`, `launch.ts`, `configKeys.ts` (generated) + `configDocs.ts`,
  `scripts/generate-config-keys.ts`, `host/` (config, status parse and store, rollout reader,
...
event→status mapping, the `transcript_path` it keeps, and the launch line's quoting.
`host/rollout.test.ts` pins the rollout read (totals from the last `token_count` with cached input split
out, the last `update_plan` winning over other calls, appended lines picked up with a half-written one held
back, malformed lines and plans ignored); `host/index.test.ts` pins a hook's rollout reaching the pushed
status, `web/store.test.ts` that a push without them keeps the last totals and plan, and
`e2e/plugins/codex/codex-status.spec.ts` the chip and plan in the terminal accessory.
