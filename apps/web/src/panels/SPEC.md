...
a project picker, the prompt hero, and the reused
  ...
  **auto-fix toggle** (`review-autofix-toggle`, a switch over `store.reviewAutoFix` →
  ...
  built to grow. `ProvidersSettings`/`AppearanceSettings`/`ChatSettings`/`ClaudeCodeSettings`/
  `TemplatesSettings`/
...
- **The effort chip is a switch too**, and the same one: `lib.driveEffortPicker` steers `/effort` to the
  ...
  by its highlight — which is `lib`'s problem, not this module's.
...
The pane is three surfaces over one snapshot — Context, Settings, Capabilities — and each lives in its
own file (the context surface stays in `ClaudeConfigPanel`, the other two are `ClaudeSettingsSurface`
and `ClaudeCapabilitiesSurface`), sharing row furniture through `ClaudeConfigParts` (scope chip, source
link, row action, the open-source hook). `ClaudeConfigPanel` owns the snapshot, the surface switch and
the one pending edit; the surfaces never talk to the transport.
