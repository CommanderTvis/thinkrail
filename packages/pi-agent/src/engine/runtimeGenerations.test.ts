...
import { defaultSessionDirFor, writeFixtureSession } from "./piSessionFixtures";
...
async function liveSession() {
	...
	const session = await createSession({ cwd, model: toEngineModel(faux.getModel()) });
...
describe("PI runtime generations", () => {
	...
	test("resolves live-chat model changes against that chat's retained generation", async () => {
		const session = await liveSession();
		const model = toEngineModel(faux.getModel());
		...
		await expect(createSession({ cwd, model })).rejects.toThrow("Unknown or unavailable model");
	});
	...
	test("a legacy disk transcript with no persisted model may use the current default", async () => {
		...
		const hydrated = await getSessionMessages(fixture.id, cwd);
