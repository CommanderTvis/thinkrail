...
- `hiddenModels` defaults to an empty list when absent; an update must be an array of strings, trimmed and deduplicated before persistence and broadcast. Wire mutations with non-string elements or a non-array value are rejected before cache, persistence, or broadcast changes.
