...
export function ClaudeAccountSurface({ revision, reloads }: { revision: number; reloads: number }) {
	const [account, setAccount] = useState<ClaudeAccount | null>(null);
	const [error, setError] = useState<string | null>(null);
	const [busy, setBusy] = useState(false);
	// A press asks Claude Code for current numbers; a file-change tick only re-reads what it wrote.
	...
	useEffect(() => {
		...
		getTransport()
			.request("claudeConfig.account", refresh ? { refresh: true } : {})
			.then((next) => {
