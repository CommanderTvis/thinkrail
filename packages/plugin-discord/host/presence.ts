...
import { DISCORD_APPLICATION_ID, type DiscordPresence, type DiscordStatus } from "../contracts";
import type { DiscordActivity } from "./ipc";

/** The plugin's settings, defaulted — never the raw, partial `PluginSettings<typeof discordContract>`. */
...
export function decidePresence(
	...
	if (!DISCORD_APPLICATION_ID.test(settings.applicationId))
		return { kind: "silent", detail: "Add a Discord application id to start publishing." };
...
export function statusFor(
	...
	if (decision.kind === "silent")
		return { state: "unconfigured", published: null, detail: decision.detail };
