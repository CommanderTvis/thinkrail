...
- **Public surface (barrel):** `resolveClaudeConfig(workspaceId, root)`, `planClaudeEdit` /
  ...
  `moveClaudePlugin`, `marketplaceCommand` / `runMarketplaceAction`, `parseMcpList` /
  `listClaudeMcpServers` / `mcpListCapabilities`.
...
- **Usage is read from Claude's cache, and refreshed through Claude** (`account.ts`). The percentages
  live in `cachedUsageUtilization`, which Claude Code writes when it fetches them; reading the file alone
  shows whatever it last saw. A refresh therefore runs `claude -p "/usage"` — the same slash command a
  session would, in print mode — and reads the file afterwards. It carries its own longer budget, because
  it is a round trip to the service rather than a file read, and it happens only when a client asks for
  it (`claudeConfig.account { refresh: true }`): the tick-driven read stays a file read.
- **The servers no file declares come from Claude itself** (`mcpList.ts`). claude.ai connectors are
  account state Claude fetches at runtime — on disk there is only `claudeAiMcpEverConnected`, a list of
  names that were *ever* connected, which still names connectors long since removed — and a plugin's
  servers live inside the plugin. Neither is anything a file walk can report, so the pane asks
  `claude mcp list` (bounded, cwd-scoped like every other CLI call: "disabled for this project" is a
  per-worktree fact) and parses its `<name>: <target> - <glyph> <status>` lines; a name may contain
  spaces, so it ends at the first `: ` and the status starts at the last ` - `. Names a file already
  declares are dropped, since the resolver's rows for those carry a real origin. The rest wear `user`
  scope with **no path**, which is how the pane knows there is nothing to edit — their switch is `/mcp`
  (or the plugin), so the row offers none — and a connector `/mcp` disabled for this project reports
