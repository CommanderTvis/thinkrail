...
once enabled it stays silent while `applicationId` is empty: Discord always shows the *registering
application's* name, so there is no anonymous "some app is running" state to fall back to. The default is
**ThinkRail's own registered application**, so enabling the integration is the only step — a user who
would rather publish under their own name registers one at discord.com/developers/applications and pastes
its id in, and one who clears the field gets the silent state back. `decidePresence` in `presence.ts` returns a `"silent"` verdict for both cases, which
